...
<body class="bg-white font-sans text-zinc-900 antialiased flex min-h-screen overflow-hidden">
    ...
    <div class="hidden lg:flex w-1/2 bg-[#09090b] relative items-center justify-center overflow-hidden p-12">
        ...
        <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
            <div class="absolute -top-20 -left-20 w-[500px] h-[500px] bg-gradient-to-r from-emerald-500 via-blue-500 to-orange-500/20 rounded-full blur-[120px] animate-blob"></div>
        ...
        <div class="relative z-10 w-full max-w-lg animate-fade-in-up">
            <div class="mb-10 inline-flex items-center justify-center p-3 bg-white/95 border border-zinc-200 rounded-2xl backdrop-blur-xl shadow-2xl">
                <img src="{{ asset('logohelix.png') }}" alt="Logo" class="h-12 w-auto object-contain drop-shadow-lg">
            </div>
            <h1 class="text-5xl font-black text-white leading-[1.1] tracking-tight mb-6">
                Gerbang Utama<br>
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-blue-400 to-orange-400">Pusat Produk UMKM.</span>
            </h1>
            ...
            <div class="bg-white/5 border border-white/10 backdrop-blur-md p-6 rounded-3xl flex items-center gap-4">
                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-500 via-blue-500 to-orange-500 flex items-center justify-center text-white text-xl shadow-lg shrink-0">
        ...
        <div class="w-full max-w-md animate-fade-in-up" style="animation-delay: 0.1s;">
            ...
            <div class="mb-10">
                ...
                <p class="text-zinc-500 font-medium text-sm">
                    Belum punya akun? <a href="{{ route('register') }}" class="text-emerald-600 font-bold hover:underline transition-all">Daftar sekarang</a>
                </p>
            </div>
            ...
            <form action="{{ route('login.process') }}" method="POST" class="space-y-5">
                ...
                <div class="relative group">
                    ...
                    <input type="text" id="username" name="username" placeholder="Email atau Username" required value="{{ old('username') }}" {{ ($sisaDetik ?? 0) > 0 ? 'disabled' : '' }} class="w-full bg-zinc-50 border border-zinc-200 text-black text-sm font-semibold rounded-2xl focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 block pl-11 pr-4 py-4 transition-all outline-none placeholder:text-zinc-400 placeholder:font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                </div>
                ...
                <div class="relative group">
                    ...
                    <input type="password" id="password" name="password" placeholder="Kata Sandi" required {{ ($sisaDetik ?? 0) > 0 ? 'disabled' : '' }} class="w-full bg-zinc-50 border border-zinc-200 text-black text-sm font-semibold rounded-2xl focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 block pl-11 pr-12 py-4 transition-all outline-none placeholder:text-zinc-400 placeholder:font-medium disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <a href="{{ route('password.request') }}" 
       class="text-xs font-bold text-zinc-500 hover:text-emerald-600 transition-colors">
       Lupa Kata Sandi?
    </a>
</div>

                {{-- Submit Button --}}
                <button type="submit" {{ ($sisaDetik ?? 0) > 0 ? 'disabled' : '' }} class="w-full bg-black hover:bg-gradient-to-r from-emerald-500 via-blue-500 to-orange-500 text-white font-black py-4 rounded-2xl transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_8px_30px_rgba(37,99,235,0.3)] hover:-translate-y-1 disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:bg-black disabled:cursor-not-allowed mt-2">
                    Masuk ke Dashboard
                </button>
