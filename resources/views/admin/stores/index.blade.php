...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            ...
            <span class="text-emerald-600 dark:text-emerald-400">Kelola Toko & Kasta (Tier)</span>
        </div>
    </div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm overflow-hidden transition-colors duration-300">
    ...
    <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300">
        <div class="text-xs font-bold text-slate-500 dark:text-slate-400">
            Menampilkan <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $stores->firstItem() ?? 0 }}</span> - <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $stores->lastItem() ?? 0 }}</span> dari <span class="text-slate-800 dark:text-white font-black">{{ $stores->total() }}</span> toko
        </div>
        ...
        <div class="modal-content">
            <form id="formTier" method="POST" action="">
                ...
                <div class="flex gap-3 justify-end mt-2 border-t border-slate-100 dark:border-slate-800 p-5">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl font-bold text-sm text-white bg-slate-800 hover:bg-slate-900 dark:bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 dark:hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 shadow-md transition-all outline-none">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
