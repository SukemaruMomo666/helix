...
<div class="flex-1 flex justify-between items-center w-full h-full">
    ...
    <div class="flex items-center gap-3 sm:gap-5" x-data="{ profileOpen: false }">
        ...
        <div class="relative group hidden md:block">
            <i class="mdi mdi-magnify absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-500 dark:text-slate-500 dark:group-focus-within:text-white transition-colors text-lg"></i>
            <input type="text"
                   class="w-48 lg:w-72 pl-11 pr-14 py-2.5 bg-slate-100/50 dark:bg-[#0a0a0a]/50 border border-slate-200/60 dark:border-white/[0.05] text-sm text-slate-700 dark:text-slate-200 rounded-full focus:bg-white dark:focus:bg-[#000000] focus:border-blue-400 dark:focus:border-white/[0.15] focus:ring-4 focus:ring-emerald-500/10 dark:focus:ring-white/[0.05] outline-none transition-all duration-300 font-semibold placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-inner dark:shadow-none backdrop-blur-sm"
...
{{-- Profile Trigger --}}
            <button @click="profileOpen = !profileOpen" @click.outside="profileOpen = false"
                ...
                <div class="hidden md:block text-left">
                    ...
                    <div class="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mt-0.5 transition-colors duration-300">
                        Super Access
                    </div>
                </div>

                <i class="mdi mdi-chevron-down text-slate-400 dark:text-slate-500 text-lg transition-transform duration-300 hidden md:block" :class="profileOpen ? 'rotate-180 text-emerald-500 dark:text-white' : 'group-hover:text-emerald-500 dark:group-hover:text-white'"></i>
            </button>
            ...
            <div x-show="profileOpen" x-cloak
                ...
                <div class="px-5 py-4 bg-slate-50/50 dark:bg-white/[0.02] border-b border-slate-100 dark:border-white/[0.05] transition-colors duration-300">
                    ...
                    <p class="text-[10px] font-bold text-slate-500 dark:text-slate-400 truncate">{{ Auth::user()->email ?? 'admin@helix.com' }}</p>
                </div>

                {{-- Dropdown Links --}}
                <div class="p-2 space-y-1">
                    <a href="#" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-white hover:bg-emerald-50 dark:hover:bg-white/[0.05] transition-all duration-200 no-underline group">
                        <i class="mdi mdi-account-cog-outline text-lg text-slate-400 dark:text-slate-500 group-hover:text-emerald-500 dark:group-hover:text-white transition-colors"></i> Pengaturan Profil
                    </a>
                    <a href="#" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-white hover:bg-emerald-50 dark:hover:bg-white/[0.05] transition-all duration-200 no-underline group">
                        <i class="mdi mdi-shield-key-outline text-lg text-slate-400 dark:text-slate-500 group-hover:text-emerald-500 dark:group-hover:text-white transition-colors"></i> Log Keamanan
                    </a>
                </div>
