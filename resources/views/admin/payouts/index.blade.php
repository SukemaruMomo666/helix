...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            ...
            <span class="text-emerald-600 dark:text-emerald-400">Manajemen Payout (UMKM)</span>
        </div>
    </div>
    <div class="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-emerald-500/20 px-4 py-2.5 rounded-xl flex items-center gap-3 transition-colors duration-300">
        <i class="mdi mdi-bank-transfer text-indigo-500 text-xl"></i>
        <span class="text-xs font-bold text-indigo-700 dark:text-emerald-400">Pastikan transfer manual dilakukan sebelum melakukan konfirmasi di sistem.</span>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm overflow-hidden transition-colors duration-300">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-200">
                    ...
                    <td class="px-6 py-5">
                        ...
                        <div class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5 mt-0.5"><i class="mdi mdi-email-outline"></i> {{ $p->email_pemilik }}</div>
                    </td>
                    <td class="px-6 py-5">
                        <div class="p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl w-60 shadow-inner dark:shadow-none">
                            <div class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-1">{{ $p->rekening_bank ?? 'N/A' }}</div>
                    ...
                    <td class="px-6 py-5">
                        <div class="text-lg font-black text-emerald-600 dark:text-white dark:text-glow-indigo font-mono">Rp {{ number_format($p->jumlah_payout, 0, ',', '.') }}</div>
                    </td>
                    <td class="px-6 py-5">
                        @php
    ...
    <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors">
        <div class="text-xs font-bold text-slate-500 dark:text-slate-400">
            Menampilkan <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $payouts->firstItem() ?? 0 }}</span> - <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $payouts->lastItem() ?? 0 }}</span> dari <span class="text-slate-800 dark:text-white font-black">{{ $payouts->total() }}</span> permintaan
        </div>
            ...
            <form id="formProses" method="POST" action="">
                ...
                <div class="modal-body p-8 bg-slate-50/50 dark:bg-slate-900">
                    <div class="text-center mb-6">
                        ...
                        <div class="text-4xl font-black text-emerald-600 dark:text-white font-mono tracking-tighter">Rp <span id="mdl-jumlah">0</span></div>
                    </div>
                    ...
                    <div class="p-6 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm text-center mb-6">
                        <div class="text-xs font-black text-indigo-500 dark:text-emerald-400 uppercase tracking-widest mb-1" id="mdl-bank">BANK</div>
