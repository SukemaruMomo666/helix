...
@section('title', 'Moderasi Produk: ' . $produk->nama_barang)
...
<style>
    ...
    .dark .dark\:bg-emerald-500\/50 { background-color: rgba(59, 130, 246, 0.5) !important; }
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            Pusat Moderasi <i class="mdi mdi-chevron-right text-sm"></i> <span class="text-emerald-600 dark:text-emerald-400 truncate max-w-[200px]">{{ $produk->nama_barang }}</span>
        </div>
    </div>
    ...
    <div class="flex-1 w-full pt-2">
        ...
        <div class="flex flex-wrap gap-3 mt-4">
            <div class="inline-flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl">
                <div class="w-6 h-6 rounded bg-blue-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs"><i class="mdi mdi-storefront-outline"></i></div>
                <span class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ $produk->nama_toko }}</span>
            </div>

            <div class="inline-flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl">
                <div class="w-6 h-6 rounded bg-indigo-100 dark:bg-indigo-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-xs"><i class="mdi mdi-tag-outline"></i></div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start mb-10">
    ...
    <div class="lg:col-span-2 space-y-6 lg:space-y-8">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 lg:p-8 shadow-sm transition-colors duration-300">
            <h4 class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                <i class="mdi mdi-text-box-outline text-lg"></i> Deskripsi Produk
            </h4>
    ...
    <div class="lg:col-span-1 space-y-6 lg:space-y-8 sticky top-[100px]">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 shadow-sm transition-colors duration-300">
            ...
            <ul class="m-0 p-0 list-none divide-y divide-slate-100 dark:divide-slate-800/80">
                <li class="py-3 flex justify-between items-center">
                    ...
                    <span class="text-lg font-black text-emerald-600 dark:text-emerald-400">Rp {{ number_format($produk->harga, 0, ',', '.') }}</span>
                </li>
                <li class="py-3 flex justify-between items-center">
        ...
        @if($produk->status_moderasi == 'pending')
            <div class="bg-white dark:bg-slate-900 border-2 border-emerald-500/30 dark:border-emerald-500/50 rounded-[2rem] p-6 shadow-lg shadow-blue-500/10 dark:shadow-[0_0_30px_rgba(59,130,246,0.1)] relative overflow-hidden transition-colors duration-300">
                <div class="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-blue-400 via-indigo-500 to-purple-500"></div>

                <h4 class="text-[11px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em] mb-2 text-center">
                    Keputusan Eksekusi
                </h4>
...
<script>
    ...
    function confirmApprove() {
        Swal.fire({
            ...
            text: "Produk ini akan langsung tampil di katalog dan bisa dibeli oleh customer.",
            icon: 'success',
