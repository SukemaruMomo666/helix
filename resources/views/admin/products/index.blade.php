...
@section('title', 'Pusat Moderasi Produk')
...
<style>
    ...
    .dark .dark\:hover\:bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500:hover { background-color: #2563eb !important; }
    .dark .dark\:hover\:border-emerald-500:hover { border-color: #3b82f6 !important; }
    .dark .dark\:hover\:border-emerald-500\/50:hover { border-color: rgba(59, 130, 246, 0.5) !important; }
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            ...
            <span class="text-emerald-600 dark:text-emerald-400">Moderasi Produk</span>
        </div>
        <p class="text-[11px] font-bold text-slate-500 dark:text-slate-400 mt-2 m-0 max-w-xl leading-relaxed">
            Tinjau kelayakan produk dan produk yang diunggah oleh mitra toko sebelum ditayangkan ke publik untuk memastikan standar kualitas platform.
        </p>
    </div>
    ...
    <div class="p-5 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
        ...
        <div class="flex p-1 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 w-full xl:w-auto overflow-x-auto filter-wrapper shadow-inner dark:shadow-none">
            ...
            @foreach($statusTabs as $key => $data)
                <a href="{{ route('admin.products.index', ['status' => $key, 'search' => $search]) }}"
                   class="flex items-center gap-2 px-4 py-2 text-xs font-black capitalize rounded-lg transition-all text-decoration-none outline-none whitespace-nowrap {{ $status_filter == $key ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-white shadow-sm border border-slate-200 dark:border-slate-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
        ...
        <form action="{{ route('admin.products.index') }}" method="GET" class="relative w-full xl:w-96 m-0">
            ...
            <input type="text" name="search" value="{{ $search }}"
                   ...
                   placeholder="Cari nama produk, merek, atau toko...">
        </form>
    </div>
</div>
        ...
        <div class="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 rounded-2xl flex flex-col overflow-hidden shadow-sm hover-lift group transition-all duration-300 hover:border-blue-300 dark:hover:border-emerald-500/50 dark:hover:shadow-[0_10px_30px_-10px_rgba(59,130,246,0.15)] relative">
            ...
            <div class="p-4 flex flex-col flex-1">
                ...
                <div class="flex justify-between items-start mb-2">
                    <span class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded">{{ $prod->nama_kategori ?? 'UMUM' }}</span>
                </div>
                ...
                {{-- Action Button --}}
                <a href="{{ route('admin.products.show', $prod->id) }}" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-slate-800/80 hover:bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:text-white dark:hover:bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-slate-700 dark:text-slate-300 text-xs font-black rounded-xl transition-all text-decoration-none outline-none group/btn mb-4 border border-slate-200 dark:border-slate-700 hover:border-blue-600 dark:hover:border-emerald-500">
...
<div class="px-6 py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300 shadow-sm">
    <div class="text-xs font-bold text-slate-500 dark:text-slate-400">
        Menampilkan <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $products->firstItem() ?? 0 }}</span> - <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $products->lastItem() ?? 0 }}</span> dari <span class="text-slate-800 dark:text-white font-black">{{ $products->total() }}</span> produk
    </div>
