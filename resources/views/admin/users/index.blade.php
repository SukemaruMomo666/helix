...
<style>
    ...
    .dark .dark\:bg-emerald-500\/10 { background-color: rgba(59, 130, 246, 0.15) !important; }
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            ...
            <span class="text-emerald-600 dark:text-emerald-400">Kelola Pengguna</span>
        </div>
    </div>
    <div class="flex items-center gap-3">
        ...
        @if(auth()->user()->admin_role === 'super')
            <button class="px-5 py-2.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-600/30 transition-all outline-none flex items-center gap-2" data-bs-toggle="modal" data-bs-target="#modalAddAdmin">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    <div class="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 p-5 rounded-2xl flex items-center gap-4 shadow-sm hover-lift transition-colors duration-300 group">
        <div class="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-2xl flex-shrink-0 group-hover:scale-110 transition-transform">
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden transition-colors duration-300">
        ...
        <div class="flex p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 w-full md:w-auto overflow-x-auto hide-scrollbar">
            @foreach(['semua', 'admin', 'seller', 'customer'] as $lv)
                <a href="{{ route('admin.users.index', ['level' => $lv, 'search' => $search]) }}"
                   class="px-4 py-2 text-xs font-black capitalize rounded-lg transition-all text-decoration-none outline-none whitespace-nowrap {{ $level_filter == $lv ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-white shadow-sm border border-slate-200 dark:border-slate-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
                    {{ $lv }}
                </a>
            @endforeach
        </div>
            ...
            <thead>
                <tr class="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
                    <th class="px-6 py-4 w-12 text-center align-middle">
                        <input type="checkbox" class="w-4 h-4 text-emerald-600 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 rounded focus:ring-emerald-500">
                    </th>
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-200">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-[11px] font-bold text-slate-500 dark:text-slate-400 mt-0.5">
                                    ID: #{{ str_pad($user->id, 5, '0', STR_PAD_LEFT) }} <span class="mx-1 opacity-50">•</span> <span class="text-emerald-600 dark:text-emerald-400">@ {{ $user->username }}</span>
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 align-middle">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black tracking-widest border
                            ...
                            @else bg-emerald-50 text-emerald-600 border-blue-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20 @endif">
                    ...
                    <td class="px-6 py-4 text-right align-middle">
                        <div class="flex items-center justify-end gap-2">
                            {{-- Btn Detail --}}
                            <button type="button" class="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-blue-100 dark:border-emerald-500/20 flex items-center justify-center text-emerald-600 dark:text-emerald-400 hover:bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:text-white dark:hover:bg-emerald-500 transition-all shadow-sm outline-none btn-detail"
                                    data-nama="{{ $user->nama }}"
    ...
    <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300">
        <div class="text-xs font-bold text-slate-500 dark:text-slate-400">
            Menampilkan <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $users->firstItem() ?? 0 }}</span> - <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $users->lastItem() ?? 0 }}</span> dari <span class="text-slate-800 dark:text-white font-black">{{ $users->total() }}</span> pengguna
        </div>
        ...
        <div class="modal-content rounded-[2rem] border-0 shadow-2xl overflow-hidden transition-colors duration-300">
            <div class="modal-header p-6 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                <h5 class="font-black text-slate-800 dark:text-white flex items-center gap-2 m-0 text-base">
                    <i class="mdi mdi-shield-account-outline text-emerald-600 text-xl"></i> Tambah Administrator
                </h5>
                ...
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" class="form-control rounded-xl py-3 font-bold text-sm shadow-inner dark:shadow-none" placeholder="budiman@helix.com" required>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3 justify-end mt-2 pt-2">
                        ...
                        <button type="submit" class="w-full sm:w-auto px-8 py-3 rounded-xl font-black text-sm text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 shadow-md shadow-blue-600/20 transition-all outline-none">BUAT AKUN</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-[2rem] border-0 shadow-2xl overflow-hidden transition-colors duration-300">
            ...
            <div class="modal-body text-center pt-0 pb-8 px-6 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                ...
                <p class="text-sm font-bold text-emerald-500 dark:text-emerald-400 mb-5">@<span id="detUsername">username</span></p>
            ...
            <div class="modal-body p-6 bg-slate-50/50 dark:bg-slate-900">
                <div class="grid grid-cols-1 gap-4">
                    <div class="flex flex-col gap-1.5 p-4 bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                        ...
                        <span class="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2"><i class="mdi mdi-email-outline text-emerald-500 text-lg"></i> <span id="detEmail">email</span></span>
                    </div>
                    <div class="flex flex-col gap-1.5 p-4 bg-white dark:bg-slate-800/80 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
