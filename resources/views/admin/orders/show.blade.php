...
<style>
    ...
    .dark .dark\:bg-emerald-500\/20 { background-color: rgba(59, 130, 246, 0.2) !important; }
    .dark .dark\:border-emerald-500\/20 { border-color: rgba(59, 130, 246, 0.2) !important; }
    .dark .dark\:border-emerald-500\/30 { border-color: rgba(59, 130, 246, 0.3) !important; }
    .dark .dark\:border-emerald-500\/50 { border-color: rgba(59, 130, 246, 0.5) !important; }
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 mt-2 pl-14 transition-colors duration-300">
            <span class="text-emerald-600 dark:text-emerald-400 font-mono tracking-wider">{{ $order->kode_invoice }}</span>
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 lg:p-8 shadow-sm transition-colors duration-300 mb-8 overflow-hidden">
    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6">
        <h3 class="text-base font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-map-marker-path text-emerald-500 text-xl"></i> Lacak Status Pesanan</h3>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-sm transition-colors duration-300 overflow-hidden">
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 bg-emerald-50/30 dark:bg-transparent flex justify-between items-center">
                <div>
                    <h3 class="text-base font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-store-outline text-emerald-500 text-xl"></i> Mitra: {{ $order->nama_toko }}</h3>
                    <p class="text-[10px] font-bold text-slate-500 mt-1 mb-0 uppercase tracking-widest"><i class="mdi mdi-phone"></i> Hubungi Toko: <span class="text-emerald-600 dark:text-emerald-400">{{ $order->telp_toko ?? '-' }}</span></p>
                </div>
            </div>
                ...
                <div class="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/30 group hover:border-blue-200 dark:hover:border-slate-600 transition-colors">
                    ...
                    <div class="flex-grow">
                        ...
                        <div class="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                            {{ $item->jumlah_item }} unit <span class="mx-1">x</span> <span class="text-emerald-600 dark:text-emerald-400 font-mono tracking-tight">Rp {{ number_format($item->harga_saat_ini, 0, ',', '.') }}</span>
                        </div>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
                        ...
                        @else
                            ...
                            <div class="text-sm font-black text-emerald-600 dark:text-emerald-400 font-mono tracking-wider">{{ $order->resi_pengiriman ?? 'BELUM TERBIT' }}</div>
                        @endif
                    </div>
                </div>
    ...
    <div class="space-y-8">
        ...
        {{-- Card: Pembeli --}}
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] shadow-sm p-6 transition-colors duration-300 relative overflow-hidden group hover:border-blue-300 dark:hover:border-emerald-500/50">
            <div class="absolute right-0 top-0 w-24 h-24 bg-emerald-50 dark:bg-emerald-500/10 rounded-bl-full flex items-start justify-end p-4 transition-colors">
                <i class="mdi mdi-account-circle-outline text-3xl text-emerald-500 opacity-50 group-hover:opacity-100 group-hover:scale-110 transition-all"></i>
            </div>
            ...
            <div class="relative z-10">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <span class="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider bg-emerald-50 dark:bg-emerald-500/10 border border-blue-100 dark:border-emerald-500/20 px-2.5 py-1 rounded w-fit mt-1">Status Web: {{ $order->sumber_transaksi }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-slate-50 to-white dark:from-slate-800/80 dark:to-slate-900 border border-slate-200 dark:border-slate-700 rounded-[2rem] shadow-sm p-6 lg:p-8 transition-colors duration-300 relative overflow-hidden group">
            ...
            {{-- Highlight Total --}}
            <div class="bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 dark:bg-emerald-500/20 border border-blue-600 dark:border-emerald-500/50 rounded-2xl p-5 relative overflow-hidden shadow-lg shadow-blue-600/20">
                <div class="absolute -right-4 -top-4 w-16 h-16 bg-white/10 rounded-full blur-xl pointer-events-none"></div>

                <div class="flex flex-col gap-1 mb-4 border-b border-white/20 dark:border-emerald-500/30 pb-4">
                    <div class="flex justify-between items-center">
                        ...
                        @if($order->tipe_pembayaran == 'DP')
                            <span class="px-2 py-0.5 bg-amber-500 text-white text-[9px] font-black uppercase rounded shadow-sm">UMKM DP SYSTEM</span>
                        @endif
                    </div>
