...
<style>
    ...
    .dark .dark\:border-emerald-500\/20 { border-color: rgba(59, 130, 246, 0.2) !important; }
    .dark .dark\:hover\:bg-emerald-500\/20:hover { background-color: rgba(59, 130, 246, 0.2) !important; }
    .dark .dark\:hover\:bg-emerald-500\/10:hover { background-color: rgba(59, 130, 246, 0.1) !important; }
    .dark .dark\:hover\:border-emerald-500\/30:hover { border-color: rgba(59, 130, 246, 0.3) !important; }
    ...
    .dark .dark\:border-emerald-500\/20 { border-color: rgba(99, 102, 241, 0.2) !important; }
</style>
@endpush
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            ...
            <span class="text-emerald-600 dark:text-emerald-400">Pantau Pesanan Global</span>
        </div>
    </div>
    <div class="bg-emerald-50 dark:bg-emerald-500/10 border border-blue-100 dark:border-emerald-500/20 px-4 py-2.5 rounded-xl flex items-center gap-3 transition-colors duration-300">
        <i class="mdi mdi-radar text-emerald-500 text-xl animate-pulse"></i>
        <span class="text-xs font-bold text-blue-700 dark:text-emerald-400">Sistem memantau seluruh pergerakan transaksi, logistik, dan aliran dana secara real-time.</span>
    </div>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    ...
    <div class="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 p-5 rounded-2xl flex items-center justify-between shadow-sm hover-lift transition-colors duration-300 relative overflow-hidden group">
        ...
        <div class="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-emerald-500 flex items-center justify-center text-2xl flex-shrink-0 group-hover:scale-110 transition-all duration-300 relative z-10">
...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] shadow-sm overflow-hidden transition-colors duration-300">
    ...
    <div class="p-5 border-b border-slate-100 dark:border-slate-800/80 bg-white dark:bg-slate-900 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 transition-colors duration-300">
        ...
        <div class="flex flex-col gap-3 w-full xl:w-auto">
            ...
            <div class="flex p-1 bg-emerald-50/50 dark:bg-emerald-500/5 rounded-xl border border-blue-100 dark:border-emerald-500/20 w-fit overflow-x-auto filter-wrapper">
                @php
                    $sources = [
                ...
                @foreach($sources as $val => $label)
                    <a href="{{ route('admin.orders.index', ['sumber' => $val, 'status' => $status, 'search' => $search]) }}"
                       class="px-4 py-2 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all text-decoration-none outline-none whitespace-nowrap {{ $sumber == $val ? 'bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white shadow-md' : 'text-emerald-600 dark:text-emerald-400 hover:bg-blue-100 dark:hover:bg-emerald-500/10' }}">
            ...
            <div class="flex p-1 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 w-full xl:w-auto overflow-x-auto filter-wrapper">
                ...
                @foreach($tabs as $val => $label)
                    <a href="{{ route('admin.orders.index', ['status' => $val, 'sumber' => $sumber, 'search' => $search]) }}"
                       class="px-4 py-2 text-xs font-black capitalize rounded-lg transition-all text-decoration-none outline-none whitespace-nowrap {{ $status == $val ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-white shadow-sm border border-slate-200 dark:border-slate-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-200">
                    ...
                    <td class="px-6 py-5 align-top">
                        <div class="flex items-center gap-2 mb-3">
                            <a href="{{ route('admin.orders.show', $order->id) }}" class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 text-blue-700 dark:text-emerald-400 border border-blue-100 dark:border-emerald-500/20 rounded-lg text-xs font-black font-mono hover:bg-blue-100 dark:hover:bg-emerald-500/20 transition-colors text-decoration-none shadow-sm dark:shadow-none">
                            ...
                            @else
                                <span class="px-2 py-1 bg-emerald-500 text-white text-[9px] font-black uppercase rounded shadow-sm" title="Online Transaction">
                        ...
                        <div class="mb-2">
                            @php
                                ...
                                elseif($st == 'diproses') $stClass = 'bg-emerald-50 text-emerald-600 border-blue-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20';
                                elseif($st == 'dikirim') $stClass = 'bg-indigo-50 text-emerald-600 border-indigo-200 dark:bg-indigo-500/10 dark:text-emerald-400 dark:border-emerald-500/20';
                        ...
                        @if(($order->tipe_pembayaran ?? 'LUNAS') == 'DP')
                            <div class="p-3 bg-amber-50/50 dark:bg-amber-500/5 border border-dashed border-amber-300 dark:border-amber-500/30 rounded-xl w-64">
                                <strong class="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest block mb-2 flex items-center gap-1.5">
                                    <i class="mdi mdi-star-circle text-sm"></i> Transaksi UMKM (DP)
                                </strong>
    ...
    <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4 transition-colors duration-300">
        <div class="text-xs font-bold text-slate-500 dark:text-slate-400">
            Menampilkan <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $orders->firstItem() ?? 0 }}</span> - <span class="text-emerald-600 dark:text-emerald-400 font-black">{{ $orders->lastItem() ?? 0 }}</span> dari <span class="text-slate-800 dark:text-white font-black">{{ $orders->total() }}</span> transaksi
        </div>
