...
<style>
    ...
    .dark .dark\:bg-emerald-500\/10 { background-color: rgba(59, 130, 246, 0.1) !important; }
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <div class="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-400 transition-colors duration-300">
            <a href="{{ route('admin.dashboard') }}" class="hover:text-emerald-600 dark:hover:text-blue-400 transition-colors text-decoration-none">Dashboard</a>
            <i class="mdi mdi-chevron-right text-sm"></i>
            <span class="text-emerald-600 dark:text-emerald-400">Konfigurasi Platform</span>
        </div>
    ...
    <div class="flex flex-col lg:flex-row gap-8 pb-32 items-start relative">
        ...
        <div class="w-full lg:w-72 flex-shrink-0 lg:sticky lg:top-[90px] z-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 rounded-2xl shadow-sm transition-colors duration-300">
            <div class="nav flex flex-col space-y-1" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                ...
                @foreach($tabs as $index => $tab)
                    <button class="nav-link w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-black transition-all outline-none text-left {{ $index === 0 ? 'active bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shadow-inner dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-800 dark:hover:text-slate-200' }}"
        ...
        <div class="flex-grow w-full max-w-4xl tab-content" id="v-pills-tabContent">
            ...
            <div class="tab-pane fade show active" id="panel-general" role="tabpanel">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 lg:p-8 shadow-sm transition-colors duration-300">
                    ...
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6 mt-2">
                        <h3 class="text-xl font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-card-account-details-outline text-emerald-500"></i> Profil Platform</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="app_name" value="{{ $settings['app_name'] ?? 'Helix' }}" class="form-control-custom p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white shadow-inner dark:shadow-none">
                        </div>
                        <div>
                            ...
                            <input type="email" name="support_email" value="{{ $settings['support_email'] ?? '' }}" class="form-control-custom p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white shadow-inner dark:shadow-none" placeholder="cs@helix.com">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="input-group">
                                <span class="input-group-text bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 border-r-0"><i class="mdi mdi-facebook text-emerald-600 text-lg"></i></span>
            ...
            <div class="tab-pane fade" id="panel-frontend" role="tabpanel">
                ...
                <div class="bg-white dark:bg-slate-900 border-t-4 border-t-blue-500 border-x border-b border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-sm transition-colors duration-300 mb-8">
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6">
                        <h3 class="text-xl font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-monitor-dashboard text-emerald-500"></i> Banner Utama Website (Slider)</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        @for($i = 1; $i <= 4; $i++)
                            ...
                            <div class="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 transition-colors">
                                <label class="block text-[11px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                    <span class="w-5 h-5 rounded bg-blue-100 dark:bg-emerald-500/20 flex items-center justify-center">{{ $i }}</span> Slide Banner
                                </label>
                                ...
                                <label for="{{ $imgKey }}Input" class="img-preview-box aspect-[21/9] md:aspect-[3/1] mb-4 border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-emerald-500 dark:hover:border-blue-400 rounded-xl overflow-hidden relative flex items-center justify-center cursor-pointer bg-white dark:bg-slate-900 transition-colors">
                                    ...
                                    <div id="{{ $imgKey }}Placeholder" class="text-center z-20 p-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-colors" style="{{ $imgSrc ? 'display: none;' : 'display: block;' }}">
                                        <i class="mdi mdi-cloud-upload-outline text-2xl text-emerald-500 mb-1"></i>
                ...
                <div class="bg-white dark:bg-slate-900 border-t-4 border-t-amber-500 border-x border-b border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-sm transition-colors duration-300 mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            @endphp
                            <label for="popupImageInput" class="img-preview-box aspect-[3/4] max-w-xs mx-auto md:mx-0 border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-emerald-500 rounded-xl overflow-hidden relative flex items-center justify-center cursor-pointer bg-white dark:bg-slate-900 transition-colors">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex justify-between items-center p-4 border border-slate-200 dark:border-slate-700 rounded-xl">
                            <div>
                                ...
                                <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400">Otomatis menampilkan produk dengan penjualan terbanyak.</span>
                            </div>
                            <div>
            ...
            <div class="tab-pane fade" id="panel-finance" role="tabpanel">
                ...
                <div class="bg-white dark:bg-slate-900 border-t-4 border-t-blue-500 border-x border-b border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-sm transition-colors duration-300 mb-8">
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6">
                        <h3 class="text-xl font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-percent-circle-outline text-emerald-500"></i> Skema Komisi Mitra (Seller)</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label class="flex justify-between items-center text-[11px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-2 ml-1">
                                ...
                                <button type="button" class="info-btn btn-show-info flex items-center justify-center w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-purple-50 hover:text-purple-500 dark:hover:bg-purple-500/20 dark:hover:text-purple-400 transition-all duration-300 outline-none group" data-title="Official Store" data-desc="Distributor resmi (PT/CV) yang menjual partai besar UMKM. Disarankan komisi tertinggi (3% - 5%) karena platform memberikan mereka akses langsung ke pesanan kakap.">
                                    <i class="mdi mdi-help text-xs group-hover:scale-110 transition-transform"></i>
                                </button>
                            </label>
                            <div class="input-group">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 mt-2 mb-0 ml-1 text-center">Distributor / Pabrik UMKM.</p>
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-900 border-t-4 border-t-emerald-500 border-x border-b border-slate-200 dark:border-slate-800 rounded-3xl p-6 lg:p-8 shadow-sm transition-colors duration-300">
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6">
                        <h3 class="text-xl font-black text-slate-800 dark:text-white m-0 flex items-center gap-2"><i class="mdi mdi-handshake text-emerald-500"></i> Sistem UMKM & Uang Muka (DP)</h3>
                        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 mb-0">Atur syarat pembelian pesanan skala besar dengan sistem cicilan awal (DP).</p>
                    </div>
            ...
            <div class="tab-pane fade" id="panel-api" role="tabpanel">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 lg:p-8 shadow-sm transition-colors duration-300">
                    <div class="border-b border-slate-100 dark:border-slate-800 pb-5 mb-6">
                        <h3 class="text-xl font-black text-slate-800 dark:text-white m-0 flex items-center gap-2">
                            <i class="mdi mdi-api text-emerald-500"></i> Biteship API
                        </h3>
            ...
            <div class="tab-pane fade" id="panel-catalog" role="tabpanel">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 lg:p-8 shadow-sm transition-colors duration-300">
                    ...
                    <div class="flex justify-between items-center p-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl mb-4 transition-colors duration-300">
                        <div class="pr-4">
                            <strong class="block text-sm font-black text-slate-800 dark:text-white mb-1">Auto-Approve Produk Baru</strong>
...
<div class="save-bar bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 p-4 lg:p-5 flex justify-end shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.5)] transition-colors duration-300">
    <button type="submit" form="mainSettingsForm" class="flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white text-sm font-black rounded-xl shadow-lg shadow-blue-600/30 hover:-translate-y-1 transition-all outline-none w-full sm:w-auto justify-center">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-[2rem] border-0 shadow-2xl overflow-hidden transition-colors duration-300">
            <div class="modal-header border-b border-slate-100 dark:border-slate-800 p-6 bg-white dark:bg-slate-900">
                <h5 class="text-lg font-black text-emerald-600 dark:text-emerald-400 m-0" id="infoModalTitle">
            ...
            <div class="modal-footer border-t border-slate-100 dark:border-slate-800 p-6 bg-white dark:bg-slate-900">
                <button type="button" class="w-full px-5 py-3 rounded-xl font-black text-sm text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 shadow-md shadow-blue-600/20 transition-all outline-none" data-bs-dismiss="modal">SAYA PAHAM</button>
            </div>
        </div>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        tabLinks.forEach(link => {
            link.addEventListener('shown.bs.tab', function(e) {
                tabLinks.forEach(l => {
                    l.classList.remove('bg-emerald-50', 'dark:bg-emerald-500/10', 'text-emerald-600', 'dark:text-emerald-400', 'shadow-inner', 'dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]');
                    l.classList.add('text-slate-500', 'dark:text-slate-400');
                });

                e.target.classList.add('bg-emerald-50', 'dark:bg-emerald-500/10', 'text-emerald-600', 'dark:text-emerald-400', 'shadow-inner', 'dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]');
