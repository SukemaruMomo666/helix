...
<div class="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8 font-sans text-slate-900 space-y-6">
    ...
    <div class="relative bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 overflow-hidden">
        ...
        <div class="absolute -right-20 -top-20 w-72 h-72 bg-emerald-50 rounded-full blur-3xl pointer-events-none"></div>

        <div class="relative z-10 flex items-center gap-5">
            <div class="w-16 h-16 bg-emerald-50 border border-blue-100 rounded-2xl flex items-center justify-center text-emerald-600 shadow-inner">
    ...
    <div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <a href="{{ route('seller.orders.index', ['status' => 'diproses']) }}" class="group bg-white border border-slate-200 rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-blue-500/10 hover:border-blue-300 relative overflow-hidden">
                ...
                <div class="text-3xl md:text-4xl font-black text-slate-800 mb-2 group-hover:text-emerald-600 transition-colors">{{ $perlu_diproses }}</div>
                <div class="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                    <div class="p-1 bg-emerald-50 text-emerald-500 rounded-md"><i class="mdi mdi-package-variant text-base leading-none"></i></div>
                    Pesanan Masuk
                </div>
            </a>
        ...
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    ...
                    <select id="chartFilter" class="bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold rounded-xl px-4 py-2 hover:bg-slate-100 focus:ring-2 focus:ring-emerald-500 focus:outline-none cursor-pointer transition-colors">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 hover:border-blue-100 transition-colors">
                        ...
                        <div class="text-lg md:text-xl font-black text-emerald-600 truncate">Rp {{ number_format($total_penjualan ?? 0, 0, ',', '.') }}</div>
                    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col h-full">
                <h4 class="text-sm font-black text-slate-400 uppercase tracking-widest mb-5 flex items-center gap-2 border-b border-slate-100 pb-4">
                    <i class="mdi mdi-podium-gold text-amber-500 text-lg"></i> Produk Terlaris
                </h4>
                            ...
                            @foreach($top_produk_keys as $index => $nama)
                                <div class="flex justify-between items-center group">
                                    <div class="flex items-center gap-3 overflow-hidden">
                                        {{-- Rank Badge --}}
                                        <div class="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 text-xs font-black group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors flex-shrink-0">
                                            {{ $index + 1 }}
                                        </div>
                                        <span class="text-sm font-bold text-slate-700 truncate group-hover:text-emerald-600 transition-colors">{{ $nama }}</span>
                                    </div>
                                    <div class="text-xs font-black bg-emerald-50 text-emerald-600 px-3 py-1 rounded-lg whitespace-nowrap ml-2">
            ...
            {{-- Info UMKM Premium Box --}}
            <div class="bg-emerald-50 border border-blue-200 rounded-3xl overflow-hidden relative shadow-sm">
                <div class="absolute top-0 left-0 w-full h-1 bg-emerald-500"></div>
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 rounded-xl bg-blue-100 text-emerald-600 flex items-center justify-center flex-shrink-0 shadow-inner">
                            <i class="mdi mdi-truck-flatbed text-2xl"></i>
                        </div>
                        <div>
                            <h5 class="text-sm font-black text-slate-900 mb-1">Armada Sendiri (UMKM)</h5>
                            <p class="text-xs font-medium text-slate-600 leading-relaxed">
                                Aktifkan "Armada Toko" di Pengaturan Pengiriman untuk memudahkan pembeli memesan produk curah/berat.
                            </p>
                            <a href="{{ route('seller.pengaturan.pengiriman') }}" class="inline-flex items-center gap-1 mt-3 text-xs font-bold text-emerald-600 hover:text-blue-800 transition-colors bg-white px-3 py-1.5 rounded-lg border border-blue-100 shadow-sm hover:shadow">
