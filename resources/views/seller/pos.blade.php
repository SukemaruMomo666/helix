...
@section('title', 'Helix POS - Terminal Kasir Cerdas')
    ...
    <div class="pos-sidebar">
        <div class="sidebar-header">
            <div>
                ...
                <div class="status-badge" style="margin-top: 4px;">
                    <div class="dot"></div> Online • {{ $tokoPos->nama_toko ?? 'Kasir Helix' }}
                </div>
            </div>
        ...
        <div class="cart-items" id="cart-items">
            <div id="empty-cart-message" style="margin: auto; text-align: center; color: var(--pos-text-muted);">
                ...
                <p style="font-weight: 700; font-size: 13px;">Belum ada produk terpilih</p>
            </div>
        </div>
        ...
        <div class="sidebar-footer">
            ...
            <div class="payment-area">
                <div class="payment-method-area" style="margin-bottom: 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                    ...
                    <button class="btn-method dp-btn" data-method="DP UMKM" onclick="setPaymentMethod('DP UMKM')" style="background: var(--pos-card); color: var(--pos-warning); border-color: var(--pos-warning);">
                ...
                <div id="dp-info-area" style="display: none; padding: 10px; border-radius: 8px; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.3); margin-bottom: 10px;">
                    <div style="font-size: 11px; font-weight: bold; color: var(--pos-warning); margin-bottom: 5px;">
                        <i class="fas fa-info-circle"></i> UMKM Uang Muka (DP)
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderProducts(products) {
        ...
        if(products.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 100px; color: var(--pos-text-muted);"><i class="fas fa-search fa-2x" style="opacity: 0.2;"></i><p style="margin-top: 15px; font-weight: 600;">Produk tidak ditemukan</p></div>`;
            return;
        }
        products.forEach(p => {
    ...
    window.setPaymentMethod = function(method) {
        ...
        document.querySelectorAll('.btn-method').forEach(btn => {
            ...
            if (btn.getAttribute('data-method') === method) {
                btn.style.background = method === 'DP UMKM' ? 'var(--pos-warning)' : 'var(--pos-primary)';
    ...
    function calculateChange() {
        ...
        if(currentTotal === 0) {
            changeDisplay.innerText = method === 'DP UMKM' ? "Sisa Tagihan: Rp 0" : "Kembali: Rp 0";
    ...
    document.getElementById('process-payment-btn').addEventListener('click', function() {
        ...
        if (method === 'DP UMKM') {
            if (currentTotal < dpMinTotal) {
                Swal.fire({icon: 'error', title: 'Belum Memenuhi Syarat', text: 'Total belanja harus minimal Rp ' + formatRp(dpMinTotal) + ' untuk menggunakan fitur DP UMKM.'}); 
                return;
            }
            let minDp = currentTotal * (dpPercent / 100);
            if (paid < minDp) {
        ...
        .then(data => {
            if(data.status === 'success') {
                let htmlContent = method === 'DP UMKM' 
                ...
                Swal.fire({
                    title: method === 'DP UMKM' ? 'DP BERHASIL DICATAT' : 'TRANSAKSI BERHASIL',
                    html: htmlContent,
                    icon: 'success',
