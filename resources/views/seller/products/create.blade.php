    ...
    <form id="productForm" action="{{ isset($product) ? route('seller.products.update', $product->id) : route('seller.products.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm">
                ...
                <div>
                    ...
                    <div class="text-sm font-medium text-slate-500 flex items-center gap-2 mt-1">
                        ...
                        <a href="{{ route('seller.products.index') }}" class="hover:text-emerald-600 transition-colors">Katalog</a>
                        <i class="mdi mdi-chevron-right text-xs"></i>
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                ...
                <button type="submit" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white font-bold rounded-xl shadow-sm shadow-blue-600/20 transition-all">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3 bg-slate-50/50">
                        <div class="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg"><i class="mdi mdi-information-variant leading-none"></i></div>
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <input type="text" id="inputNama" name="nama_barang" value="{{ old('nama_barang', $product->nama_barang ?? '') }}" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder-slate-400" placeholder="Contoh: Kripik Singkong Balado 100gr" required>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2">Kategori <span class="text-red-500">*</span></label>
                                <select name="kategori_id" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all cursor-pointer" required>
...
<div>
    ...
    <div class="flex">
        ...
        <select id="inputSatuan" name="satuan_unit" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-r-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all cursor-pointer" required>
            <option value="">-- Pilih Satuan --</option>
            <optgroup label="Umum & Eceran">
                ...
                <option value="Pasang" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Pasang') ? 'selected' : '' }}>Pasang</option>
            </optgroup>
            <optgroup label="Kemasan & Grosir">
                ...
                <option value="Box" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Box') ? 'selected' : '' }}>Box</option>
                <option value="Lusin" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Lusin') ? 'selected' : '' }}>Lusin (12 Pcs)</option>
                <option value="Kodi" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Kodi') ? 'selected' : '' }}>Kodi (20 Pcs)</option>
                <option value="Gross" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Gross') ? 'selected' : '' }}>Gross (144 Pcs)</option>
            </optgroup>
            <optgroup label="Berat & Volume">
                <option value="Gram" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Gram') ? 'selected' : '' }}>Gram (g)</option>
                <option value="Kg" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Kg') ? 'selected' : '' }}>Kilogram (Kg)</option>
                <option value="Ml" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Ml') ? 'selected' : '' }}>Mililiter (ml)</option>
                <option value="Liter" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Liter') ? 'selected' : '' }}>Liter (L)</option>
            </optgroup>
            <optgroup label="Panjang & Lainnya">
                <option value="Cm" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Cm') ? 'selected' : '' }}>Centimeter (cm)</option>
                <option value="Meter" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Meter') ? 'selected' : '' }}>Meter (m)</option>
                <option value="Roll" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Roll') ? 'selected' : '' }}>Roll / Gulung</option>
                <option value="Lembar" {{ (old('satuan_unit', $product->satuan_unit ?? '') == 'Lembar') ? 'selected' : '' }}>Lembar / Helai</option>
            </optgroup>
        </select>
    </div>
</div>
                ...
                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3 bg-slate-50/50">
                        <div class="p-1.5 bg-indigo-50 text-emerald-600 rounded-lg"><i class="mdi mdi-image-multiple leading-none"></i></div>
                    ...
                    <div class="p-6">
                        ...
                        <div id="uploadZone" class="border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50 hover:bg-emerald-50 hover:border-blue-400 transition-colors p-10 text-center cursor-pointer group relative overflow-hidden">
                            <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-slate-100 group-hover:scale-110 transition-transform relative z-10">
                                <i class="mdi mdi-cloud-upload-outline text-3xl text-emerald-500"></i>
                            </div>
                        ...
                        <div id="previewGrid" class="flex flex-wrap gap-4 mt-6 empty:mt-0">
                            @if(isset($product) && $product->gambar_utama)
                                <div class="relative w-24 h-24 rounded-xl overflow-hidden border border-emerald-500 group shadow-md" id="previewLama">
                                    <img src="{{ asset('assets/uploads/products/' . $product->gambar_utama) }}" class="w-full h-full object-cover">
                                    <div class="absolute bottom-0 left-0 w-full bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white text-[9px] font-bold text-center py-0.5">COVER LAMA</div>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div>
                                ...
                                <div class="flex">
                                    ...
                                    <input type="text" id="viewHarga" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm font-bold rounded-r-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder-slate-400" placeholder="0" required>
                            ...
                            <div>
                                ...
                                <input type="number" name="min_order" value="{{ old('min_order', $product->min_order ?? '1') }}" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder-slate-400" placeholder="1" required min="1">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2">Stok Tersedia <span class="text-red-500">*</span></label>
                                <input type="number" name="stok" value="{{ old('stok', $product->stok ?? '') }}" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder-slate-400" placeholder="0" required min="0">
                            </div>
                        </div>
                        ...
                        @endphp
                        <div class="bg-emerald-50/50 border border-blue-100 rounded-2xl p-5">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-blue-100 text-emerald-600 flex items-center justify-center"><i class="mdi mdi-tag-percent-outline"></i></div>
                                    <div>
                            ...
                            <div id="boxDiskon" class="{{ $hasDiscount ? 'block' : 'hidden' }} mt-4 pt-4 border-t border-blue-200/50 space-y-4">
                                <div class="flex flex-col sm:flex-row gap-4">
                                    <select id="typeDiskon" name="tipe_diskon" class="w-full sm:w-1/3 bg-white border border-blue-200 text-slate-900 text-sm font-bold rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none cursor-pointer shadow-sm" {{ !$hasDiscount ? 'disabled' : '' }}>
                                    ...
                                    <input type="number" id="valDiskon" name="nilai_diskon" value="{{ old('nilai_diskon', $product->nilai_diskon ?? '') }}" class="w-full sm:w-2/3 bg-white border border-blue-200 text-slate-900 text-sm font-bold rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none placeholder-slate-400 shadow-sm" placeholder="Masukkan nilai diskon (Cth: 15)">
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Berlaku Mulai</label>
                                        <input type="datetime-local" name="diskon_mulai" value="{{ old('diskon_mulai', $product->diskon_mulai ?? '') }}" class="w-full bg-white border border-blue-200 text-slate-900 text-sm font-bold rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none shadow-sm cursor-pointer">
                                    </div>
                                    <div>
                                        <label class="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Berakhir Pada</label>
                                        <input type="datetime-local" name="diskon_berakhir" value="{{ old('diskon_berakhir', $product->diskon_berakhir ?? '') }}" class="w-full bg-white border border-blue-200 text-slate-900 text-sm font-bold rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none shadow-sm cursor-pointer">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {{-- 4. SPESIFIKASI & LOGISTIK PENGIRIMAN --}}
                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3 bg-slate-50/50">
                        <div class="p-1.5 bg-amber-50 text-amber-600 rounded-lg"><i class="mdi mdi-truck-fast leading-none"></i></div>
                        <div>
                            <h2 class="font-bold text-slate-800">Detail & Dimensi (Pengiriman)</h2>
                            <p class="text-[10px] font-bold text-slate-500 m-0">Wajib diisi akurat agar ongkos kirim ekspedisi bisa dihitung.</p>
                        </div>
                    </div>
                    <div class="p-6 space-y-5">
                        ...
                        <div class="p-5 border border-amber-200 bg-amber-50/30 rounded-2xl space-y-4">
                            ...
                            <div class="border-t border-amber-200 pt-4">
                                <label class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                    Dimensi Kemasan (P x L x T) cm <span class="text-[9px] bg-amber-500 text-white px-2 py-0.5 rounded font-black tracking-widest">PENTING</span>
                                </label>
                                ...
                                <p class="text-[10px] font-bold text-slate-500 mt-2 mb-0">Jika ukuran kemasan besar namun ringan (seperti Bantal, Helm, Mainan Besar), ekspedisi akan menghitung ongkir menggunakan rumus <b>Berat Volumetrik</b> ini.</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Deskripsi Lengkap <span class="text-red-500">*</span></label>
                            <textarea id="inputDeskripsi" name="deskripsi" rows="6" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-3 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder-slate-400 leading-relaxed" placeholder="Jelaskan spesifikasi detail, kegunaan, dan info pengiriman..." required>{{ old('deskripsi', $product->deskripsi ?? '') }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="w-full h-[650px] bg-slate-50 border-[10px] border-slate-900 rounded-[40px] relative shadow-2xl overflow-hidden">
                        ...
                        <div class="w-full h-full overflow-y-auto hide-scrollbar pb-20 bg-slate-100 relative">
                            ...
                            <div class="bg-white p-4 mb-2">
                                ...
                                <h1 class="text-sm font-bold text-slate-800 mt-2 leading-snug" id="hpTitle">{{ $product->nama_barang ?? 'Nama produk akan tampil di sini...' }}</h1>
                            </div>
                            ...
                            <div class="bg-white p-4 min-h-[200px]">
                                <h3 class="text-xs font-black text-slate-900 mb-3 pb-2 border-b border-slate-100">Spesifikasi Produk</h3>
                        ...
                        <div class="absolute bottom-0 left-0 w-full h-[60px] bg-white border-t border-slate-200 flex items-center px-3 gap-2 z-40">
                            ...
                            <div class="flex-1 h-10 flex items-center justify-center text-emerald-600 border border-blue-600 rounded-lg text-xs font-black bg-emerald-50">+ Keranjang</div>
                            <div class="flex-1 h-10 flex items-center justify-center text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 rounded-lg text-xs font-black shadow-md shadow-blue-500/30">Beli Langsung</div>
                        </div>
                    </div>
        ...
        function updateFiles() {
            ...
            if (dt.files.length > 0) {
                ...
                Array.from(dt.files).forEach((file, index) => {
                    ...
                    reader.onload = (e) => {
                        let badge = index === 0 ? '<div class="absolute bottom-0 left-0 w-full bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500/90 text-white text-[10px] font-bold text-center py-0.5">COVER BARU</div>' : '';
                        let html = `
                            <div class="relative w-24 h-24 rounded-xl overflow-hidden border border-emerald-500 group shadow-md">
