...
@section('title', 'Katalog Produk (Etalase)')
...
<div class="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8 font-sans text-slate-900 space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-shrink-0">
            ...
            <a href="{{ route('seller.products.create') }}" class="flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white text-sm font-bold rounded-xl shadow-sm shadow-blue-600/20 transition-all">
                <i class="mdi mdi-plus-box-outline text-lg leading-none"></i> Tambah Produk
            </a>
        </div>
    </div>
        ...
        <form action="{{ route('seller.products.index') }}" method="GET" class="bg-slate-50/50 p-4 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div class="relative w-full flex-1 group">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i class="mdi mdi-magnify text-slate-400 group-focus-within:text-emerald-500 transition-colors text-lg"></i>
                </div>
                <input type="text" name="search" class="w-full pl-11 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all shadow-sm" placeholder="Cari nama produk atau SKU..." value="{{ request('search') }}">
            </div>

            <div class="flex items-center gap-3 w-full sm:w-auto">
                <select name="status" class="w-full sm:w-48 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl px-4 py-2.5 hover:bg-slate-50 focus:ring-2 focus:ring-emerald-500 outline-none cursor-pointer transition-colors shadow-sm" onchange="this.form.submit()">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-4 min-w-[320px]">
                                    ...
                                    <div>
                                        <h6 class="text-sm font-bold text-slate-900 leading-snug mb-1.5 line-clamp-2 group-hover:text-emerald-600 transition-colors">{{ $produk->nama_barang }}</h6>
                                        <div class="flex flex-wrap gap-1.5">
                                            ...
                                            {{-- Cek Dimensi Kargo (Super Detail UMKM) --}}
                                            ...
                                            @else
                                                <span class="inline-block font-mono text-[10px] font-bold text-red-600 bg-red-50 border border-red-200 px-2 py-0.5 rounded shadow-sm cursor-help" title="Dimensi belum diisi! Ongkir ekspedisi bisa error."><i class="mdi mdi-alert"></i> Dimensi Kosong!</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="py-4 px-6 whitespace-nowrap">
                                ...
                                <div class="text-sm font-black text-emerald-600">Rp {{ number_format($hargaTampil, 0, ',', '.') }}</div>
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('seller.products.edit', $produk->id) }}" class="w-9 h-9 rounded-xl flex items-center justify-center bg-white border border-slate-200 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 hover:border-blue-200 transition-all shadow-sm" title="Edit Data">
                        ...
                        <tr>
                            <td colspan="6" class="py-16 text-center">
                                <div class="flex flex-col items-center justify-center opacity-60">
                                    ...
                                    <p class="text-sm font-medium text-slate-500">Belum ada produk yang ditambahkan atau sesuai filter.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform scale-95 transition-transform duration-300" id="importModalContent">
        ...
        <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h3 class="text-lg font-black text-slate-800 flex items-center gap-2">
                <i class="mdi mdi-file-excel text-emerald-600 text-2xl"></i> Import Produk Baru
            </h3>
        ...
        {{-- Modal Body --}}
        <div class="p-6">
            <div class="bg-emerald-50 border border-blue-100 rounded-2xl p-4 flex gap-3 mb-6">
                <i class="mdi mdi-information text-emerald-500 text-xl shrink-0"></i>
                <div class="text-sm text-blue-800 font-medium leading-relaxed">
                    Semua produk yang di-import akan otomatis masuk ke gudang dengan status <b class="font-black">Off Etalase</b>. Ini berguna jika barang hanya untuk stok POS Kasir offline.
                </div>
            </div>
            ...
            <form action="{{ route('seller.products.import') }}" method="POST" enctype="multipart/form-data" id="formImportExcel">
                ...
                <div id="dropZone" class="border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center hover:bg-slate-50 hover:border-blue-400 transition-colors cursor-pointer group" onclick="document.getElementById('fileExcel').click()">
                    <i class="mdi mdi-cloud-upload-outline text-5xl text-slate-300 group-hover:text-emerald-500 transition-colors mb-2 block"></i>
        ...
        document.querySelectorAll('.btn-delete-confirm').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Produk ini akan ditarik dari etalase. Data tidak dihapus permanen untuk menjaga riwayat invoice pesanan pembeli terdahulu.",
                    icon: 'warning',
