    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <div class="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm relative overflow-hidden">
                <div class="absolute -right-20 -bottom-20 w-64 h-64 bg-emerald-50 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm flex flex-col">
                <div class="flex gap-6 px-6 pt-4 border-b border-slate-100 overflow-x-auto hide-scrollbar bg-slate-50/50">
                    <a href="?tab=dilepas" class="pb-3 text-sm font-bold whitespace-nowrap transition-colors border-b-2 {{ $tab == 'dilepas' ? 'border-blue-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-800' }}">Dana Masuk (Selesai)</a>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($transaksi_list as $tx)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    <td class="py-4 px-6 font-mono font-black text-emerald-600 text-sm whitespace-nowrap">{{ $tx->kode_invoice }}</td>
            ...
            <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                ...
                @forelse($riwayat_payout as $rp)
                    <div class="py-3.5 flex justify-between items-center border-b border-slate-50 last:border-0">
                        ...
                        <span class="px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest {{ $rp->status == 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600' }}">
                            {{ $rp->status }}
                        </span>
                    </div>
                @empty
            ...
            <div id="modalPanel" class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all opacity-0 scale-95 duration-300 w-full sm:max-w-lg border border-slate-200">
                ...
                <form action="{{ route('seller.finance.payout') }}" method="POST" id="payoutForm">
                    ...
                    <div class="bg-slate-50 px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                        <h3 class="text-lg font-black text-slate-900 flex items-center gap-2"><i class="mdi mdi-bank-transfer-out text-emerald-600"></i> Tarik Saldo Toko</h3>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        @else
                            <div class="bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white p-5 rounded-2xl shadow-inner relative overflow-hidden">
                            ...
                            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 flex items-start gap-3">
                                <i class="mdi mdi-information text-emerald-500 text-xl leading-none"></i>
