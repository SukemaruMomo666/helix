...
<div class="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8 font-sans text-slate-900 space-y-6">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        ...
        <div class="flex bg-white border border-slate-200 rounded-2xl p-2 shadow-sm w-full xl:w-auto overflow-x-auto hide-scrollbar">
            <div class="px-6 py-2 border-r border-slate-100 min-w-[140px]">
                ...
                <h3 class="text-xl font-black text-emerald-600">{{ $stats['aktif'] ?? 0 }}</h3>
            </div>
    ...
    <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div class="flex gap-6 px-6 pt-4 border-b border-slate-100 overflow-x-auto hide-scrollbar bg-slate-50/50">
            <a href="?tab=semua" class="pb-3 text-sm font-bold whitespace-nowrap transition-colors border-b-2 {{ $currentTab == 'semua' ? 'border-blue-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-800' }}">Semua Voucher</a>
            <a href="?tab=aktif" class="pb-3 text-sm font-bold whitespace-nowrap transition-colors border-b-2 {{ $currentTab == 'aktif' ? 'border-blue-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-800' }}">Sedang Berjalan</a>
            <a href="?tab=habis" class="pb-3 text-sm font-bold whitespace-nowrap transition-colors border-b-2 {{ $currentTab == 'habis' ? 'border-blue-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-800' }}">Kuota Habis</a>
            <a href="?tab=nonaktif" class="pb-3 text-sm font-bold whitespace-nowrap transition-colors border-b-2 {{ $currentTab == 'nonaktif' ? 'border-blue-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-800' }}">Berakhir / Nonaktif</a>
        </div>
        ...
        <div class="p-4 sm:p-6 border-b border-slate-100 bg-white flex flex-col md:flex-row gap-4 justify-between">
            <form action="{{ route('seller.promotion.vouchers') }}" method="GET" class="relative w-full md:max-w-md group m-0">
                ...
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i class="mdi mdi-magnify text-slate-400 group-focus-within:text-emerald-600 transition-colors text-lg"></i>
                </div>
            ...
            <button type="button" onclick="openVoucherModal()" class="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white font-bold rounded-xl shadow-sm shadow-blue-600/20 transition-all flex-shrink-0">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="py-4 px-6 whitespace-nowrap">
                                @if($vch->tipe_diskon == 'PERSEN')
                                    <div class="text-base font-black text-emerald-600">Diskon {{ $vch->nilai_diskon }}%</div>
                                ...
                                @else
                                    <div class="text-base font-black text-emerald-600">Rp {{ number_format($vch->nilai_diskon, 0, ',', '.') }}</div>
            ...
            <div id="modalPanel" class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all opacity-0 scale-95 duration-300 w-full sm:max-w-2xl border border-slate-200">
                ...
                <form action="{{ route('seller.promotion.vouchers.store') }}" method="POST">
                    ...
                    <div class="bg-slate-50 px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                        <h3 class="text-lg font-black text-slate-900 flex items-center gap-2"><i class="mdi mdi-ticket-percent text-emerald-600"></i> Buat Voucher Baru</h3>
                    ...
                    <div class="bg-slate-50 px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-3 rounded-b-3xl">
                        ...
                        <button type="submit" class="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white font-bold rounded-xl shadow-sm shadow-blue-600/20 transition-all flex items-center justify-center gap-2">
