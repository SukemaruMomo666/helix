<aside class="w-[260px] h-screen bg-slate-900 border-r border-slate-800 flex flex-col fixed left-0 top-0 z-50 transition-transform duration-300" id="sidebar">
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar-dark">
        ...
        <a href="{{ route('seller.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isDashboard ? 'bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white shadow-md shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        ...
        <a href="{{ route('seller.pos.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isPos ? 'bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white shadow-md shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isPenjualanOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-package text-xl {{ $isPenjualanOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Pesanan</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isPenjualanOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isProductOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-cube-unfolded text-xl {{ $isProductOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Produk</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isProductOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isPromoOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-ticket-percent text-xl {{ $isPromoOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Pusat Promosi</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isPromoOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isServiceOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-headset-mic text-xl {{ $isServiceOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Layanan Pembeli</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isServiceOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isFinanceOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-currency-usd text-xl {{ $isFinanceOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Keuangan</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isFinanceOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isDataOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-chart-bar text-xl {{ $isDataOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Data Analitik</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isDataOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
        ...
        <div>
            <button onclick="toggleSidebarMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-bold transition-all {{ $isShopOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                <div class="flex items-center gap-3">
                    <i class="mdi mdi-store text-xl {{ $isShopOpen ? 'text-emerald-500' : 'text-slate-500' }}"></i>
                    <span>Pengaturan Toko</span>
                </div>
                <i class="mdi mdi-chevron-down text-lg transition-transform duration-300 nav-chevron {{ $isShopOpen ? 'rotate-180 text-emerald-500' : 'text-slate-500' }}"></i>
            </button>
...
<script>
    function toggleSidebarMenu(button) {
        ...
        if (content.classList.contains('grid-rows-[1fr]')) {
            ...
            chevron.classList.remove('rotate-180', 'text-emerald-500');
