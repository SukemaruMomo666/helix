        ...
        <div id="chatPanel" class="absolute md:relative top-0 left-0 w-full h-full md:flex flex-1 flex-col bg-slate-100 z-20 md:z-auto transition-transform duration-300 translate-x-full md:translate-x-0 hidden md:block">
            ...
            <div id="chatActiveWindow" class="hidden flex-col h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-slate-50">
                ...
                <div class="bg-white/90 backdrop-blur-md px-4 sm:px-6 py-3.5 border-b border-slate-200 flex items-center gap-4 shadow-sm z-10 flex-shrink-0">
                    ...
                    <div id="activeAvatar" class="w-10 h-10 rounded-full bg-blue-100 text-emerald-600 border border-blue-200 flex items-center justify-center font-black text-lg flex-shrink-0">U</div>
                ...
                <div class="bg-white border-t border-slate-200 flex flex-col flex-shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] relative z-20">
                    ...
                    <div class="flex items-center gap-3 px-5 py-2.5 bg-white border-b border-slate-100">
                        <button type="button" onclick="document.getElementById('upload-image').click()" class="text-slate-400 hover:text-emerald-600 transition-colors outline-none" title="Kirim Gambar">
                            <i class="mdi mdi-image-outline text-xl leading-none"></i>
                        </button>
                        <button type="button" onclick="document.getElementById('upload-file').click()" class="text-slate-400 hover:text-emerald-600 transition-colors outline-none" title="Kirim Dokumen">
                    ...
                    <div id="formSendMessage" class="p-4 pt-3 flex items-end gap-3">
                        ...
                        <button type="button" id="btnSendMsg" class="w-11 h-11 rounded-xl bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white flex items-center justify-center transition-colors shadow-sm shadow-blue-600/20 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed outline-none">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderContactList(chats) {
        ...
        chats.forEach(chat => {
            ...
            let isActiveClass = (id == activeChatId) ? 'bg-emerald-50/50 border-l-4 border-l-blue-600 pl-3 pr-4' : 'bg-transparent border-l-4 border-l-transparent hover:bg-slate-100/50 px-4';
    ...
    contactListDiv.addEventListener('click', function(e) {
        ...
        contactListDiv.querySelectorAll('.contact-item').forEach(el => {
            el.classList.remove('bg-emerald-50/50', 'border-l-blue-600', 'pl-3', 'pr-4');
            el.classList.add('bg-transparent', 'border-l-transparent', 'hover:bg-slate-100/50', 'px-4');
        });
        item.classList.add('bg-emerald-50/50', 'border-l-blue-600', 'pl-3', 'pr-4');
    ...
    function appendMessageUI(content, isOut, time, type = 'text', fileName = '', isRead = 0, animate = true) {
        ...
        let bubbleColor = isOut ? 'bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white rounded-l-2xl rounded-tr-2xl rounded-br-sm shadow-md' : 'bg-white border border-slate-200 text-slate-800 rounded-r-2xl rounded-tl-2xl rounded-bl-sm shadow-sm';
    ...
    function showMediaPreview(type, content, fileName) {
        ...
        if(type === 'image') {
            ...
            contentDiv.innerHTML = `<div class="h-12 w-12 bg-blue-100 text-emerald-500 rounded-lg flex items-center justify-center text-xl shadow-sm"><i class="mdi mdi-microphone"></i></div> <div class="flex flex-col"><span class="text-xs font-black text-slate-800">Voice Note</span><span class="text-[10px] text-slate-400">Siap dikirim</span></div>`;
        }
