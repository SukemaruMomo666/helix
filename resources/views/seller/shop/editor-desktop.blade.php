...
<head>
    ...
    <title>Editor Situs Web Toko - Helix</title>
...
<body x-data="desktopEditor()" x-init="initEditor()" x-cloak class="h-screen w-screen flex flex-col text-slate-800" :class="isPreviewMode ? 'bg-slate-900' : ''">
    ...
    <header class="h-16 bg-white border-b border-slate-200 flex justify-between items-center px-6 flex-shrink-0 z-30 shadow-sm transition-all duration-300">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-black text-slate-800 text-lg leading-tight flex items-center gap-2">
                    Editor Situs Web Desktop <i class="mdi mdi-check-decagram text-emerald-500 text-base" x-show="!isPreviewMode"></i>
                </h1>
        ...
        <div class="flex items-center gap-3">
            <button @click="togglePreview()" class="px-5 py-2.5 text-sm font-bold rounded-xl transition-all outline-none flex items-center gap-2" :class="isPreviewMode ? 'bg-emerald-50 text-emerald-600 border border-blue-200' : 'bg-white border border-slate-200 text-slate-700 hover:border-blue-300 hover:text-emerald-600'">
            ...
            <button x-show="!isPreviewMode" @click="tampilkan()" class="px-6 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 rounded-xl shadow-lg shadow-blue-600/30 transition-all outline-none flex items-center gap-2">
        ...
        <aside x-show="!isPreviewMode" x-transition.opacity.duration.300ms class="w-[320px] bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
            <div class="p-5 border-b border-slate-100 bg-slate-50/80">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Cari modul visual..." class="w-full pl-12 pr-4 py-3 text-sm font-bold border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-blue-100 focus:border-emerald-500 outline-none transition-all placeholder:font-medium">
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar p-5">
                ...
                <div class="grid grid-cols-2 gap-4" x-ref="paletteList">
                    ...
                    <div @click="addComponent('banner')" class="border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center text-center cursor-grab hover:border-blue-400 hover:shadow-md transition-all bg-white group" data-type="banner">
                        <div class="w-12 h-12 bg-emerald-50 text-emerald-600 flex items-center justify-center rounded-xl mb-3 group-hover:scale-110 transition-transform"><i class="mdi mdi-image-area text-2xl"></i></div>
                    ...
                    <div @click="addComponent('carousel')" class="border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center text-center cursor-grab hover:border-indigo-400 hover:shadow-md transition-all bg-white group" data-type="carousel">
                        <div class="w-12 h-12 bg-indigo-50 text-emerald-600 flex items-center justify-center rounded-xl mb-3 group-hover:scale-110 transition-transform"><i class="mdi mdi-view-grid-plus text-2xl"></i></div>
            ...
            <div class="w-full max-w-[1200px] flex-shrink-0 bg-[#f4f6f8] rounded-xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] border border-slate-300 flex flex-col relative overflow-hidden transition-all duration-300"
                ...
                <div class="h-12 bg-slate-200 border-b border-slate-300 flex items-center px-4 gap-3 select-none sticky top-0 z-[100]">
                    ...
                    <div class="ml-4 flex-1 max-w-2xl h-8 bg-white/70 rounded-lg flex items-center px-4 text-xs font-bold text-slate-500 shadow-sm border border-white">
                        <i class="mdi mdi-lock text-slate-400 mr-2 text-sm"></i> https://helix.com/{{ Str::slug($tokoName) }}
                    </div>
                </div>
                ...
                <div class="flex-1 w-full overflow-y-auto hide-scrollbar flex flex-col relative" id="canvas-scroll-container">
                    ...
                    <div class="relative h-[250px] lg:h-[300px] flex flex-col justify-end select-none flex-shrink-0 bg-cover bg-center border-b border-slate-200"
                        ...
                        <div class="relative z-10 w-full max-w-6xl mx-auto px-8 pb-8 flex items-end justify-between">
                            ...
                            <div class="flex gap-3 mb-2">
                                ...
                                <button class="px-8 py-2.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 border-2 border-blue-600 text-white text-sm font-bold rounded-lg shadow-lg hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 transition-colors"><i class="mdi mdi-plus mr-1"></i> Ikuti Toko</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm flex-shrink-0">
                        <div class="max-w-6xl mx-auto flex px-4">
                            <div class="px-8 py-4 text-center text-[15px] font-black border-b-[3px] border-blue-600 text-emerald-600 cursor-pointer">Halaman Depan</div>
                            ...
                            <template x-for="(item, index) in canvasItems" :key="item.uid">
                                <div class="canvas-item group/canvas w-full shadow-sm hover:shadow-md"
                                    ...
                                    <div x-show="item.type === 'video'" class="w-full flex flex-col bg-white rounded-xl p-6">
                                        <h4 class="text-lg font-black mb-5 px-2 truncate w-full border-l-4 border-emerald-500 pl-3"
                                            :style="`color: ${item.config.textColor}`"
                                    ...
                                    <div x-show="item.type === 'kategori'" style="display: none;" class="p-6 bg-white rounded-xl">
                                        <h4 class="text-lg font-black mb-6 border-l-4 border-emerald-500 pl-3 truncate w-full"
                                            :style="`color: ${item.config.textColor}`"
                                            x-text="item.config.title" x-show="item.config.title"></h4>

                                        <div class="grid grid-cols-6 md:grid-cols-8 gap-4">
                                            <div class="flex flex-col items-center gap-3 cursor-pointer group/cat"><div class="w-20 h-20 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center border border-blue-100 group-hover/cat:bg-emerald-500 group-hover/cat:text-white transition-all group-hover/cat:shadow-md"><i class="mdi mdi-tshirt-crew text-4xl"></i></div><span class="text-sm font-bold text-slate-600 group-hover/cat:text-emerald-600">Pakaian</span></div>
                                    ...
                                    <div x-show="item.type === 'produk'" style="display: none;" class="py-6 px-4 bg-white rounded-xl">
                                        <div class="flex justify-between items-center mb-6">
                                            <h4 class="text-xl font-black uppercase tracking-wide truncate flex-1 border-l-4 border-emerald-500 pl-3"
                                                :style="`color: ${item.config.textColor}`"
                                                x-text="item.config.title"></h4>
                                            <span class="text-sm font-bold text-emerald-600 hover:text-blue-800 cursor-pointer transition-colors flex items-center">Lihat Semua Kategori Ini <i class="mdi mdi-chevron-right text-lg"></i></span>
                                        </div>
                                                ...
                                                <template x-if="item.config.productSource === 'manual' && item.config.selectedProducts.length > 0">
                                                    <template x-for="prod in item.config.selectedProducts" :key="prod.id">
                                                        <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white hover:shadow-lg transition-all cursor-pointer group/prod min-w-[200px] w-[200px] flex-shrink-0">
                                                            ...
                                                            <div class="p-4">
                                                                <div class="text-sm font-bold text-slate-700 truncate mb-1.5 group-hover/prod:text-emerald-600 transition-colors" x-text="prod.name"></div>
        ...
        <aside x-show="!isPreviewMode" x-transition.opacity.duration.300ms class="w-[360px] bg-white border-l border-slate-200 flex flex-col flex-shrink-0 z-20 shadow-[-4px_0_24px_rgba(0,0,0,0.03)]">
            ...
            <div class="p-6 overflow-y-auto flex-1 hide-scrollbar bg-white">
                ...
                <div x-show="!activeItem">
                    <div class="bg-emerald-50 border border-blue-100 p-4 rounded-xl mb-6 flex gap-3">
                    ...
                    <div class="w-full aspect-[21/9] border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center relative cursor-pointer hover:border-emerald-500 hover:bg-emerald-50 transition-colors group rounded-2xl overflow-hidden mb-4 bg-cover bg-center"
                        ...
                        <div class="relative z-10 flex flex-col items-center w-full h-full justify-center transition-opacity" :class="uploadedHeader ? 'opacity-0 hover:opacity-100 bg-black/60 backdrop-blur-sm' : ''">
                            <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 mb-2 group-hover:scale-110 group-hover:text-emerald-600 transition-all text-slate-400">
                ...
                <div x-show="activeItem !== null" style="display: none;" x-data="{ currentConfig: {} }" x-effect="if(activeItem) currentConfig = activeItem.config">
                    ...
                    <div class="mb-6 bg-slate-50 border border-slate-200 p-5 rounded-xl">
                        ...
                        <input type="text" x-model="currentConfig.title" @input="updateItemConfig()" class="w-full px-4 py-3 border border-slate-300 rounded-lg bg-white text-sm font-bold text-slate-800 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-400 mb-4" placeholder="Kosongkan jika tanpa judul">
                    ...
                    <div x-show="activeItem && activeItem.type === 'banner'">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="currentConfig.ratio = '4:1'; updateItemConfig()" class="flex-1 py-2 text-xs font-bold rounded shadow-sm outline-none transition-all" :class="currentConfig.ratio === '4:1' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">Lebar (4:1)</button>
                                <button @click="currentConfig.ratio = '3:1'; updateItemConfig()" class="flex-1 py-2 text-xs font-bold rounded outline-none transition-all" :class="currentConfig.ratio === '3:1' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">Medium (3:1)</button>
                                <button @click="currentConfig.ratio = '16:9'; updateItemConfig()" class="flex-1 py-2 text-xs font-bold rounded outline-none transition-all" :class="currentConfig.ratio === '16:9' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">Klasik (16:9)</button>
                            </div>
                        </div>
                        ...
                        <div class="mb-5">
                            ...
                            <button x-show="!currentConfig.images || currentConfig.images.length < 3"
                                    ...
                                    class="w-full py-4 border-2 border-dashed border-blue-300 bg-emerald-50/50 text-emerald-600 text-sm font-bold rounded-xl hover:bg-blue-100 hover:border-blue-400 transition-colors flex flex-col items-center justify-center gap-1 outline-none">
                    ...
                    <div x-show="activeItem && activeItem.type === 'carousel'">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="changeGridType('3')" class="flex-1 py-3 text-[11px] font-bold rounded shadow-sm outline-none transition-all flex flex-col items-center gap-1.5" :class="currentConfig.gridType === '3' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">
                                ...
                                <button @click="changeGridType('4')" class="flex-1 py-3 text-[11px] font-bold rounded outline-none transition-all flex flex-col items-center gap-1.5" :class="currentConfig.gridType === '4' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                                ...
                                <button @click="changeGridType('5')" class="flex-1 py-3 text-[11px] font-bold rounded outline-none transition-all flex flex-col items-center gap-1.5" :class="currentConfig.gridType === '5' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                        ...
                        <div class="mb-5">
                            ...
                            <button x-show="!currentConfig.images || currentConfig.images.length < currentConfig.maxImages"
                                    @click="$refs.componentUploader.click()"
                                    class="w-full py-4 border-2 border-dashed border-blue-300 bg-emerald-50/50 text-emerald-600 text-sm font-bold rounded-xl hover:bg-blue-100 hover:border-blue-400 transition-colors flex items-center justify-center gap-2 outline-none">
                    ...
                    <div x-show="activeItem && activeItem.type === 'produk'">
                        <div class="mb-6">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="currentConfig.layout = 'horizontal'; updateItemConfig()" class="flex-1 py-3 text-xs font-bold rounded outline-none transition-all flex flex-col items-center gap-1.5" :class="currentConfig.layout === 'horizontal' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                                    <i class="mdi mdi-gesture-swipe-horizontal text-xl"></i> Scroll Samping (Max 10)
                                </button>
                                <button @click="currentConfig.layout = 'vertical'; updateItemConfig()" class="flex-1 py-3 text-xs font-bold rounded shadow-sm outline-none transition-all flex flex-col items-center gap-1.5" :class="currentConfig.layout === 'vertical' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">
                        ...
                        <div class="flex flex-col gap-3 mb-6">
                            <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-colors" :class="currentConfig.productSource === 'auto' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-slate-300'">
                                <input type="radio" name="prod_src" value="auto" x-model="currentConfig.productSource" @change="updateItemConfig()" class="w-5 h-5 text-emerald-600 focus:ring-emerald-500">
                                <div>
                                    <div class="text-sm font-black" :class="currentConfig.productSource === 'auto' ? 'text-blue-700' : 'text-slate-700'">Pilih Otomatis</div>
                                    <div class="text-[10px] font-bold mt-0.5" :class="currentConfig.productSource === 'auto' ? 'text-emerald-500/80' : 'text-slate-500'">Sistem memunculkan produk terlaris otomatis</div>
                                </div>
                            </label>

                            <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-colors" :class="currentConfig.productSource === 'manual' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-slate-300'">
                                <input type="radio" name="prod_src" value="manual" x-model="currentConfig.productSource" @change="updateItemConfig()" class="w-5 h-5 text-emerald-600 focus:ring-emerald-500">
                                <div class="flex-1">
                                    <div class="text-sm font-black" :class="currentConfig.productSource === 'manual' ? 'text-blue-700' : 'text-slate-700'">Pilih Manual</div>
                                    <div class="text-[10px] font-bold mt-0.5" :class="currentConfig.productSource === 'manual' ? 'text-emerald-500/80' : 'text-slate-500'">Anda dapat memilih maksimal 10 produk.</div>
                                </div>
                            </label>
                        </div>
                    ...
                    <div x-show="activeItem && activeItem.type === 'video'">
                        ...
                        <div class="flex gap-2 mb-5 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                            ...
                            <button @click="currentConfig.videoSource = 'local'; updateItemConfig()" class="flex-1 py-2 text-xs font-bold rounded outline-none transition-all flex items-center justify-center gap-1.5" :class="currentConfig.videoSource === 'local' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'"><i class="mdi mdi-monitor-arrow-down text-lg"></i> File Lokal</button>
                        </div>
                        ...
                        <div x-show="currentConfig.videoSource === 'local'">
                            ...
                            <div class="w-full aspect-video border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center hover:border-emerald-500 hover:bg-emerald-50 transition-colors cursor-pointer rounded-2xl group overflow-hidden mb-4 relative" @click="$refs.videoUploader.click()">
                                ...
                                <div class="relative z-10 flex flex-col items-center p-3 text-center" :class="currentConfig.videoFile ? 'opacity-0 hover:opacity-100 bg-black/60 w-full h-full justify-center backdrop-blur-sm' : ''">
                                    <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 mb-2 group-hover:scale-110 transition-all" :class="currentConfig.videoFile ? 'text-white bg-transparent border-white/50' : 'text-emerald-500'">
                        ...
                        <div class="bg-emerald-50 border border-blue-100 p-4 rounded-xl flex gap-3">
                            <i class="mdi mdi-information text-emerald-500 text-lg"></i>
    ...
    <div x-show="showProductModal" class="fixed inset-0 z-[100005] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" style="display: none;" x-cloak>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col overflow-hidden transform transition-all modal-enter" @click.outside="showProductModal = false">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                    ...
                    <template x-for="prod in availableProducts" :key="prod.id">
                        <div class="bg-white border-2 rounded-xl p-3 cursor-pointer transition-all flex flex-col hover:shadow-md"
                             :class="isProductTempSelected(prod) ? 'border-emerald-500 shadow-blue-500/20' : 'border-slate-200 hover:border-blue-300'"
                            ...
                            <div class="aspect-square bg-slate-100 rounded-lg mb-3 overflow-hidden border border-slate-100 relative">
                                ...
                                {{-- Ceklis Overlay --}}
                                <div x-show="isProductTempSelected(prod)" class="absolute top-2 right-2 w-7 h-7 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white">
            ...
            <div class="px-8 py-5 border-t border-slate-200 bg-white flex justify-between items-center">
                <div class="text-sm font-bold text-slate-500">Total Terpilih: <span class="text-emerald-600 font-black text-lg ml-1" x-text="tempSelectedProducts.length"></span> <span class="text-slate-400 font-medium">/ 10 Produk</span></div>
                <div class="flex gap-3">
                    ...
                    <button @click="saveProductSelection()" class="px-8 py-2.5 rounded-xl font-bold text-sm text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 shadow-lg shadow-blue-500/30 transition-colors flex items-center gap-2 outline-none">
