...
<head>
    ...
    <title>Editor Dekorasi Toko - Helix</title>

    {{-- Font & Icons --}}
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@mdi/font@7.2.96/css/produkdesignicons.min.css">
...
<body x-data="pondasiEditor()" x-init="initEditor()" x-cloak class="h-screen w-screen flex flex-col text-slate-800" :class="isPreviewMode ? 'preview-mode' : ''">
    ...
    <header class="h-16 bg-white border-b border-slate-200 flex justify-between items-center px-6 flex-shrink-0 z-30 shadow-sm transition-all duration-300">
        <div class="flex items-center gap-4">
            <a x-show="!isPreviewMode" href="{{ route('seller.shop.decoration.template') }}" class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-emerald-50 text-slate-600 hover:text-emerald-600 transition-colors outline-none">
            ...
            <div>
                <h1 class="font-black text-slate-800 text-lg leading-tight flex items-center gap-2">
                    Editor Visual Toko <i class="mdi mdi-check-decagram text-emerald-500 text-base" x-show="!isPreviewMode"></i>
                </h1>
                <div class="text-[11px] font-bold text-slate-500 flex items-center gap-1">
                    Mode Aktif: <span class="text-emerald-600 uppercase tracking-wider" x-text="isPreviewMode ? 'PRATINJAU (PREVIEW)' : templateName"></span>
                </div>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <button x-show="!isPreviewMode" @click="gantiTemplate()" class="px-4 py-2 text-sm font-bold text-slate-600 bg-white border border-slate-200 hover:border-blue-300 hover:text-emerald-600 rounded-xl transition-all outline-none flex items-center gap-2">
                <i class="mdi mdi-swap-horizontal text-lg"></i> Ganti Template
            </button>
            <button @click="togglePreview()" class="px-4 py-2 text-sm font-bold rounded-xl transition-all outline-none flex items-center gap-2" :class="isPreviewMode ? 'bg-emerald-50 text-emerald-600 border border-blue-200' : 'bg-white border border-slate-200 text-slate-600 hover:border-blue-300 hover:text-emerald-600'">
            ...
            <button x-show="!isPreviewMode" @click="tampilkan()" class="px-6 py-2 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 rounded-xl shadow-lg shadow-blue-600/20 transition-all outline-none flex items-center gap-2">
    ...
    <main class="flex-1 flex min-h-0 relative">
        ...
        <aside x-show="!isPreviewMode" x-transition.opacity.duration.300ms class="w-[300px] bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20 shadow-[4px_0_24px_rgba(0,0,0,0.03)]">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Cari Modul..." class="w-full pl-10 pr-4 py-2.5 text-sm font-medium border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-blue-100 focus:border-emerald-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar">
                <div class="p-4">
                    ...
                    <div class="grid grid-cols-2 gap-3" id="palette-list">
                        <div class="border border-slate-200 rounded-2xl p-3 flex flex-col items-center justify-center text-center cursor-move hover:border-emerald-500 hover:shadow-md transition-all bg-white group" data-type="banner">
                            <div class="w-10 h-10 bg-emerald-50 text-emerald-600 flex items-center justify-center rounded-xl mb-2 group-hover:scale-110 transition-transform"><i class="mdi mdi-image-area text-xl"></i></div>
                            <span class="text-[10px] font-black text-slate-700">Banner Toko</span>
                        </div>
                        <div class="border border-slate-200 rounded-2xl p-3 flex flex-col items-center justify-center text-center cursor-move hover:border-emerald-500 hover:shadow-md transition-all bg-white group" data-type="carousel">
                            <div class="w-10 h-10 bg-indigo-50 text-emerald-600 flex items-center justify-center rounded-xl mb-2 group-hover:scale-110 transition-transform"><i class="mdi mdi-view-grid-plus text-xl"></i></div>
        ...
        <section class="flex-1 relative overflow-y-auto flex py-10 px-4 transition-all duration-300 hide-scrollbar"
            ...
            <div class="absolute left-1/2 -translate-x-[280px] top-10 hidden xl:block">
                <div class="bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-md flex items-center gap-2">
            ...
            <div class="mockup-container">
                ...
                <div class="flex-1 w-full h-full flex flex-col bg-[#f4f6f8] overflow-y-auto hide-scrollbar relative">
                    ...
                    <div class="flex border-b border-slate-200 bg-white sticky top-0 z-40 select-none shadow-sm flex-shrink-0">
                        <div class="flex-1 py-3.5 text-center text-[13px] font-black border-b-[3px] border-blue-600 text-emerald-600">Beranda</div>
                        ...
                        <template x-for="(item, index) in canvasItems" :key="item.uid">
                            ...
                            <div class="canvas-item-wrapper group/canvas z-10"
                                ...
                                <div x-show="item.type === 'kategori'" style="display: none;" class="p-3 bg-white rounded-xl shadow-sm overflow-hidden">
                                    <h4 class="text-[11px] font-black mb-3 pl-2 border-l-2 border-emerald-500 truncate w-full"
                                        :style="{ color: item.config.textColor }"
                                        x-text="item.config.title" x-show="item.config.title"></h4>
                                    <div class="grid grid-cols-4 gap-2">
                                        <div class="flex flex-col items-center gap-1.5"><div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-500 flex items-center justify-center border border-blue-100"><i class="mdi mdi-tshirt-crew text-xl"></i></div><span class="text-[9px] font-bold text-slate-600">Pakaian</span></div>
                                ...
                                <div x-show="item.type === 'produk'" style="display: none;" class="py-3 px-1 bg-transparent overflow-hidden">
                                    <div class="flex justify-between items-center mb-3 px-2">
                                        <h4 class="text-[11px] font-black uppercase tracking-wide truncate flex-1 border-l-2 border-emerald-500 pl-2"
                                            :style="{ color: item.config.textColor }"
                                            x-text="item.config.title"></h4>
                                        <span class="text-[9px] font-bold text-emerald-600 px-2 py-1 rounded bg-blue-100/50 flex-shrink-0 ml-2">Lihat Semua</span>
                                    </div>
        ...
        <aside x-show="!isPreviewMode" x-transition.opacity.duration.300ms class="w-[340px] bg-white border-l border-slate-200 flex flex-col flex-shrink-0 z-20 shadow-[-4px_0_24px_rgba(0,0,0,0.03)]">
            ...
            <div class="p-5 overflow-y-auto flex-1 hide-scrollbar bg-white">
                ...
                <div x-show="!activeItem">
                    <div class="bg-emerald-50 border border-blue-100 p-4 rounded-2xl mb-6 flex gap-3">
                        <i class="mdi mdi-information text-emerald-500 text-xl"></i>
                    ...
                    <div class="w-full aspect-[21/9] border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center relative cursor-pointer hover:border-emerald-500 hover:bg-emerald-50 transition-colors group rounded-2xl overflow-hidden mb-3 bg-cover bg-center"
                        ...
                        <div class="relative z-10 flex flex-col items-center w-full h-full justify-center transition-opacity" :class="uploadedHeader ? 'opacity-0 hover:opacity-100 bg-black/60 backdrop-blur-sm' : ''">
                            <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 mb-2 group-hover:scale-110 group-hover:text-emerald-600 transition-all text-slate-400">
                ...
                <div x-show="activeItem !== null" style="display: none;" x-data="{ currentConfig: {} }" x-effect="if(activeItem) currentConfig = activeItem.config">
                    ...
                    <div class="mb-6 bg-slate-50 border border-slate-200 p-4 rounded-xl">
                        ...
                        <input type="text" x-model="currentConfig.title" @input="updateItemConfig()" class="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-white text-sm font-bold text-slate-800 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-400 mb-3" placeholder="Kosongkan jika tanpa judul">

                        <label class="block text-[11px] font-black text-slate-500 mb-2 uppercase tracking-wider">Warna Teks</label>
                        <div class="flex gap-2">
                            <button @click="currentConfig.textColor = '#1e293b'; updateItemConfig()" class="w-8 h-8 rounded-full border-2 shadow-sm outline-none" :class="currentConfig.textColor === '#1e293b' ? 'border-emerald-500 scale-110' : 'border-slate-200 hover:scale-105'" style="background: #1e293b;"></button>
                            <button @click="currentConfig.textColor = '#ffffff'; updateItemConfig()" class="w-8 h-8 rounded-full border-2 shadow-sm outline-none" :class="currentConfig.textColor === '#ffffff' ? 'border-emerald-500 scale-110' : 'border-slate-200 hover:scale-105'" style="background: #ffffff;"></button>
                            <button @click="currentConfig.textColor = '#2563eb'; updateItemConfig()" class="w-8 h-8 rounded-full border-2 shadow-sm outline-none" :class="currentConfig.textColor === '#2563eb' ? 'border-emerald-500 scale-110' : 'border-white hover:scale-105'" style="background: #2563eb;"></button>
                            <button @click="currentConfig.textColor = '#ef4444'; updateItemConfig()" class="w-8 h-8 rounded-full border-2 shadow-sm outline-none" :class="currentConfig.textColor === '#ef4444' ? 'border-emerald-500 scale-110' : 'border-white hover:scale-105'" style="background: #ef4444;"></button>
                        </div>
                    </div>
                    ...
                    <div x-show="activeItem && activeItem.type === 'banner'">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="currentConfig.ratio = '2:1'; updateItemConfig()" class="flex-1 py-1.5 text-[10px] font-bold rounded shadow-sm outline-none transition-all" :class="currentConfig.ratio === '2:1' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">2:1 (Default)</button>
                                <button @click="currentConfig.ratio = '16:9'; updateItemConfig()" class="flex-1 py-1.5 text-[10px] font-bold rounded outline-none transition-all" :class="currentConfig.ratio === '16:9' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">16:9</button>
                                <button @click="currentConfig.ratio = '1:1'; updateItemConfig()" class="flex-1 py-1.5 text-[10px] font-bold rounded outline-none transition-all" :class="currentConfig.ratio === '1:1' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">1:1 (Persegi)</button>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <button x-show="!currentConfig.images || currentConfig.images.length < 3"
                                    ...
                                    class="w-full py-4 border-2 border-dashed border-blue-300 bg-emerald-50/50 text-emerald-600 text-xs font-bold rounded-xl hover:bg-blue-100 hover:border-blue-400 transition-colors flex flex-col items-center justify-center gap-1 outline-none">
                    ...
                    <div x-show="activeItem && activeItem.type === 'carousel'">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="changeGridType('2')" class="flex-1 py-2 text-[10px] font-bold rounded shadow-sm outline-none transition-all flex flex-col items-center gap-1" :class="currentConfig.gridType === '2' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">
                                ...
                                <button @click="changeGridType('3')" class="flex-1 py-2 text-[10px] font-bold rounded outline-none transition-all flex flex-col items-center gap-1" :class="currentConfig.gridType === '3' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                                ...
                                <button @click="changeGridType('4')" class="flex-1 py-2 text-[10px] font-bold rounded outline-none transition-all flex flex-col items-center gap-1" :class="currentConfig.gridType === '4' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                        ...
                        <div class="mb-4">
                            ...
                            <button x-show="!currentConfig.images || currentConfig.images.length < currentConfig.maxImages"
                                    @click="$refs.componentUploader.click()"
                                    class="w-full py-3 border-2 border-dashed border-blue-300 bg-emerald-50/50 text-emerald-600 text-xs font-bold rounded-xl hover:bg-blue-100 hover:border-blue-400 transition-colors flex items-center justify-center gap-2 outline-none">
                    ...
                    <div x-show="activeItem && activeItem.type === 'produk'">
                        <div class="mb-5">
                            ...
                            <div class="flex gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                                <button @click="currentConfig.layout = 'horizontal'; updateItemConfig()" class="flex-1 py-2 text-[10px] font-bold rounded outline-none transition-all flex flex-col items-center gap-1" :class="currentConfig.layout === 'horizontal' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'">
                                    <i class="mdi mdi-gesture-swipe-horizontal text-lg"></i> Scroll Samping (Max 6)
                                </button>
                                <button @click="currentConfig.layout = 'vertical'; updateItemConfig()" class="flex-1 py-2 text-[10px] font-bold rounded shadow-sm outline-none transition-all flex flex-col items-center gap-1" :class="currentConfig.layout === 'vertical' ? 'bg-white text-emerald-600 border border-slate-200' : 'text-slate-500 hover:bg-slate-100'">
                        ...
                        <div class="flex flex-col gap-3 mb-5">
                            <label class="flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors" :class="currentConfig.productSource === 'auto' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-slate-300'">
                                <input type="radio" name="prod_src" value="auto" x-model="currentConfig.productSource" @change="updateItemConfig()" class="w-4 h-4 text-emerald-600 focus:ring-emerald-500">
                                <div>
                                    <div class="text-sm font-black" :class="currentConfig.productSource === 'auto' ? 'text-blue-700' : 'text-slate-700'">Pilih Otomatis</div>
                                    <div class="text-[9px] font-bold mt-0.5" :class="currentConfig.productSource === 'auto' ? 'text-emerald-500/80' : 'text-slate-500'">Sistem memunculkan produk terlaris otomatis</div>
                                </div>
                            </label>

                            <label class="flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-colors" :class="currentConfig.productSource === 'manual' ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-slate-300'">
                                <input type="radio" name="prod_src" value="manual" x-model="currentConfig.productSource" @change="updateItemConfig()" class="w-4 h-4 text-emerald-600 focus:ring-emerald-500">
                                <div class="flex-1">
                                    <div class="text-sm font-black" :class="currentConfig.productSource === 'manual' ? 'text-blue-700' : 'text-slate-700'">Pilih Manual</div>
                                    <div class="text-[9px] font-bold mt-0.5" :class="currentConfig.productSource === 'manual' ? 'text-emerald-500/80' : 'text-slate-500'">Anda dapat memilih maksimal 8 produk.</div>
                                </div>
                            </label>
                        </div>
                    ...
                    <div x-show="activeItem && activeItem.type === 'video'">
                        ...
                        <div class="flex gap-2 mb-4 bg-slate-50 p-1.5 rounded-lg border border-slate-200">
                            ...
                            <button @click="currentConfig.videoSource = 'local'; updateItemConfig()" class="flex-1 py-1.5 text-[11px] font-bold rounded outline-none transition-all flex items-center justify-center gap-1" :class="currentConfig.videoSource === 'local' ? 'bg-white text-emerald-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:bg-slate-100'"><i class="mdi mdi-monitor-arrow-down text-lg"></i> File Lokal</button>
                        </div>
                        ...
                        <div x-show="currentConfig.videoSource === 'local'">
                            ...
                            <div class="w-full aspect-video border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center hover:border-emerald-500 hover:bg-emerald-50 transition-colors cursor-pointer rounded-2xl group overflow-hidden mb-3 relative" @click="$refs.videoUploader.click()">
                                ...
                                <div class="relative z-10 flex flex-col items-center p-3 text-center" :class="currentConfig.videoFile ? 'opacity-0 hover:opacity-100 bg-black/60 w-full h-full justify-center backdrop-blur-sm' : ''">
                                    <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 mb-2 group-hover:scale-110 transition-all" :class="currentConfig.videoFile ? 'text-white bg-transparent border-white/50' : 'text-emerald-500'">
    ...
    <div x-show="showProductModal" class="fixed inset-0 z-[100005] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" style="display: none;" x-cloak>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col overflow-hidden transform transition-all modal-enter" @click.outside="showProductModal = false">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <template x-for="prod in availableProducts" :key="prod.id">
                        <div class="bg-white border rounded-xl p-3 cursor-pointer transition-all flex flex-col hover:shadow-md"
                             :class="isProductTempSelected(prod) ? 'border-emerald-500 ring-2 ring-blue-100' : 'border-slate-200 hover:border-blue-300'"
                            ...
                            <div class="aspect-square bg-slate-100 rounded-lg mb-3 overflow-hidden border border-slate-100 relative">
                                ...
                                {{-- Ceklis Overlay --}}
                                <div x-show="isProductTempSelected(prod)" class="absolute top-2 right-2 w-6 h-6 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 text-white rounded-full flex items-center justify-center shadow-md">
            ...
            <div class="px-6 py-4 border-t border-slate-200 bg-white flex justify-between items-center">
                <div class="text-xs font-bold text-slate-500">Terpilih: <span class="text-emerald-600 font-black text-sm" x-text="tempSelectedProducts.length"></span> / 8</div>
                <div class="flex gap-3">
                    ...
                    <button @click="saveProductSelection()" class="px-6 py-2 rounded-lg font-bold text-sm text-white bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 shadow-md shadow-blue-500/30 transition-colors flex items-center gap-2 outline-none">
