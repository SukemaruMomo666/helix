    ...
    <div class="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm flex flex-col lg:flex-row justify-between items-center gap-4">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full lg:w-auto">
            ...
            <div class="relative w-full md:w-80 group">
                ...
                <input type="text" id="orderSearchInput" class="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:bg-white outline-none transition-all" placeholder="Cari Invoice / Nama...">
            </div>
        ...
        <form action="{{ route('seller.orders.massUpdate') }}" method="POST" id="mass-shipping-form" class="w-full md:w-auto">
            ...
            <div class="flex items-center justify-between md:justify-end gap-4 bg-slate-50 md:bg-transparent p-3 md:p-0 rounded-xl border border-slate-200 md:border-none">
                <label class="flex items-center gap-2 cursor-pointer group">
                    <input type="checkbox" id="select-all-orders" class="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer transition-colors">
        ...
        @else
            ...
            @foreach($groupedOrders as $invoice => $items)
                @php
                    ...
                    $accentColor = $isPos ? 'text-orange-600' : 'text-emerald-600';
                    ...
                    <div class="divide-y divide-slate-100">
                        @foreach($items as $item)
                            @php
                                ...
                                $badgeClass = 'bg-emerald-50 text-emerald-600 border-blue-200';
                                $statusText = 'Diproses';
                                ...
                                if ($status == 'menunggu_pembayaran') { 
                                    ...
                                    $badgeClass = 'bg-indigo-50 text-emerald-600 border-indigo-200'; 
                            ...
                            <div class="order-item-row flex flex-col lg:flex-row items-start lg:items-center gap-5 p-6" data-status="{{ $status }}">
                                ...
                                <div class="hidden lg:flex w-8 justify-center flex-shrink-0">
                                    @if($status == 'siap_kirim')
                                        <input type="checkbox" name="detail_ids[]" value="{{ $item->detail_id }}" class="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer order-checkbox" form="mass-shipping-form">
                                    @endif
                                </div>
                                ...
                                <div class="flex items-start gap-4 flex-1 min-w-[280px]">
                                    {{-- Checkbox Mobile --}}
                                    @if($status == 'siap_kirim')
                                        <input type="checkbox" name="detail_ids[]" value="{{ $item->detail_id }}" class="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer order-checkbox lg:hidden mt-2" form="mass-shipping-form">
                                    @endif

                                    <img src="{{ asset('assets/uploads/products/' . ($item->gambar_utama ?? 'default.jpg')) }}" class="w-16 h-16 md:w-20 md:h-20 rounded-xl object-cover border border-slate-200 flex-shrink-0" alt="Produk">
                                    <div>
                                ...
                                <div class="w-full lg:w-56 flex-shrink-0">
                                    @if(in_array($status, ['diproses', 'siap_kirim']))
                                        ...
                                        <form action="{{ route('seller.orders.updateStatus') }}" method="POST" class="flex flex-col gap-2">
                                            ...
                                            <button type="button" onclick="confirmUpdate(this, '{{ $nextActionText }}', '{{ $nextStatus }}')" class="w-full flex items-center justify-center gap-1.5 bg-gradient-to-r from-emerald-600 via-blue-600 to-orange-500 hover:from-emerald-700 hover:via-blue-700 hover:to-orange-600 text-white shadow-md hover:shadow-lg text-sm font-bold rounded-xl px-3 py-2.5 transition-all" data-text="{{ $nextActionText }}">
