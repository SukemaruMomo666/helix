...
<header class="fixed top-0 inset-x-0 z-40 bg-white/80 backdrop-blur-xl border-b border-zinc-200 shadow-[0_4px_30px_rgba(0,0,0,0.03)] h-20 transition-all duration-500 flex items-center">
    <div class="max-w-7xl mx-auto px-4 w-full flex items-center justify-between gap-4">
       ...
       {{-- TENGAH: Search Bar Enterprise (Sembunyikan di Landing Page) --}}
        @if(!request()->is('/'))
        <form action="{{ route('search') }}" method="GET" class="flex-1 max-w-2xl relative group hidden md:block">
            <input
                ...
                id="search-input"
                name="query"
                placeholder="Cari cemilan, kerajinan, oleh-oleh..."
                value="{{ request('query') }}"
                class="w-full bg-zinc-100 border-2 border-transparent text-zinc-800 text-sm font-medium rounded-2xl focus:bg-white focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 block pl-6 pr-14 py-3 transition-all outline-none placeholder:text-zinc-400 shadow-inner group-focus-within:shadow-lg group-focus-within:shadow-emerald-600/5"
            >
            {{-- Tombol Kaca Pembesar (Submit / Enter) --}}
            <button type="submit" class="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center justify-center w-9 h-9 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 hover:scale-105 transition-all duration-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-600/50 cursor-pointer">
                <i class="fas fa-search"></i>
            </button>
        </form>
        @else
        <div class="flex-1 max-w-2xl hidden md:block"></div>
        @endif
        ...
        <nav class="flex items-center gap-3 lg:gap-6 shrink-0">
            ...
            <a href="{{ route('keranjang.index') }}" class="relative w-12 h-12 flex items-center justify-center rounded-2xl bg-zinc-50 border border-zinc-200 text-zinc-700 hover:bg-emerald-600 hover:border-emerald-600 hover:text-white transition-all duration-300 shadow-sm group">
                ...
                @else
                    <span class="absolute top-3 right-3 w-2 h-2 bg-emerald-500 rounded-full border-2 border-white opacity-0 group-hover:opacity-100 transition-opacity"></span>
                @endif
            </a>
            ...
            @else
            <a href="{{ route('login') }}" class="hidden md:flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-black text-white text-sm font-bold hover:bg-emerald-600 hover:shadow-lg hover:shadow-emerald-600/30 transition-all duration-300">
    ...
    <div id="mobile-search-dropdown" class="absolute top-full left-0 w-full bg-white border-b border-zinc-200 p-4 hidden md:hidden shadow-lg transition-all duration-300">
        <form action="{{ route('search') }}" method="GET" class="relative w-full">
            <input type="text" name="query" placeholder="Cari cemilan, kerajinan..." class="w-full bg-zinc-100 border-2 border-transparent text-zinc-800 text-sm font-medium rounded-2xl focus:bg-white focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10 block pl-5 pr-14 py-3.5 transition-all outline-none placeholder:text-zinc-400">
            <button type="submit" class="absolute right-1.5 top-1/2 transform -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-emerald-600 text-white rounded-xl shadow-sm hover:bg-emerald-700 active:scale-95 transition-all">
                <i class="fas fa-search"></i>
            </button>
        </form>
    </div>
...
<div id="modern-sidebar" class="fixed top-0 left-0 h-screen w-[85%] max-w-[360px] bg-[#09090b] z-[60] shadow-[30px_0_60px_rgba(0,0,0,0.5)] transform -translate-x-[130%] transition-transform duration-500 ease-[cubic-bezier(0.87,0,0.13,1)] flex flex-col border-r border-zinc-800">
    ...
    <div class="p-6 border-b border-zinc-800 bg-zinc-900/50 flex justify-between items-start relative overflow-hidden">
        <div class="absolute -top-10 -right-10 w-32 h-32 bg-emerald-600/20 rounded-full blur-[40px]"></div>
        ...
        @else
            <div class="flex items-center gap-4 relative z-10">
                ...
                <div>
                    ...
                    <a href="{{ route('login') }}" class="text-xs font-bold text-emerald-500 hover:text-emerald-400 mt-1 inline-block transition-colors">Sign In / Register &rarr;</a>
                </div>
            </div>
        @endauth
    ...
    <div class="flex-1 overflow-y-auto px-4 py-8 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
        <div class="mb-8">
            ...
            <ul class="space-y-1.5">
                <li>
                    <a href="{{ url('/') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-zinc-400 font-semibold hover:bg-emerald-600/10 hover:text-white transition-all group border border-transparent hover:border-emerald-500/20">
                        <div class="w-9 h-9 rounded-xl bg-zinc-800/50 group-hover:bg-emerald-600 text-zinc-500 group-hover:text-white flex items-center justify-center transition-all shadow-sm group-hover:shadow-[0_0_15px_rgba(5,150,105,0.4)]">
                ...
                <li>
                    <a href="{{ route('produk.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-zinc-400 font-semibold hover:bg-emerald-600/10 hover:text-white transition-all group border border-transparent hover:border-emerald-500/20">
                        <div class="w-9 h-9 rounded-xl bg-zinc-800/50 group-hover:bg-emerald-600 text-zinc-500 group-hover:text-white flex items-center justify-center transition-all shadow-sm group-hover:shadow-[0_0_15px_rgba(5,150,105,0.4)]">
                ...
                <li>
                    <a href="{{ route('toko.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-zinc-400 font-semibold hover:bg-emerald-600/10 hover:text-white transition-all group border border-transparent hover:border-emerald-500/20">
                        <div class="w-9 h-9 rounded-xl bg-zinc-800/50 group-hover:bg-emerald-600 text-zinc-500 group-hover:text-white flex items-center justify-center transition-all shadow-sm group-hover:shadow-[0_0_15px_rgba(5,150,105,0.4)]">
                ...
                <ul class="space-y-1.5">
                    ...
                    <li>
                        <a href="{{ route('pesanan.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl text-zinc-400 font-semibold hover:bg-zinc-800 hover:text-white transition-all group relative overflow-hidden">
                            ...
                            Riwayat Transaksi
                            <span class="absolute right-4 bg-emerald-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full">BARU</span>
                        </a>
                    </li>

                    @if(Auth::user()->level === 'admin')
                        <li class="mt-6 pt-6 border-t border-zinc-800">
                            <a href="{{ url('/app_admin/dashboard_mimin.php') }}" class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-white font-bold bg-gradient-to-r from-emerald-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 transition-all shadow-[0_10px_20px_rgba(5,150,105,0.2)] group">
...
<nav class="md:hidden fixed bottom-0 inset-x-0 bg-white/90 backdrop-blur-xl border-t border-zinc-200 z-40 flex items-center justify-around h-[68px] pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.03)] px-2">
    ...
    <a href="{{ url('/') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isHome ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isHome) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
    ...
    <a href="{{ route('produk.index') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isKatalog ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isKatalog) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
    ...
    <a href="{{ route('toko.index') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isMitra ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isMitra) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
    ...
    <a href="{{ route('keranjang.index') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isKeranjang ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isKeranjang) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
        <div class="relative">
    ...
    @auth
    <a href="{{ route('profil.index') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isProfil ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isProfil) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
    ...
    @else
    <a href="{{ route('login') }}" class="flex flex-col items-center justify-center w-full h-full {{ $isProfil ? 'text-emerald-600' : 'text-zinc-400 hover:text-emerald-600' }} group relative">
        @if($isProfil) <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-600 rounded-b-full"></div> @endif
